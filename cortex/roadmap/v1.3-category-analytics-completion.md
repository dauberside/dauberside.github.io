...
**Categories**:
- `ops`: 運用, incident, on-call, alert, pager, slack
- `n8n`: n8n, recipe, workflow, verification node
- `cortex`: cortex, llm, digest, tomorrow.json
- `docs`: docs, documentation, readme, spec, 設計
...
**Before**:
```
Build Success Message → Read Digest → Extract Tasks → Estimate Category → Update Heatmap → Send Slack
```

**After**:
```
Build Success Message → Send Slack Notification
```
...
1. **Slack Notification**: Received successfully
   ...
   📅 Date: 2025-12-15 (JST)
   📁 File: cortex/daily/2025-12-15-digest.md
   ⏰ Generated at: 2025-12-14T15:21:43.593Z
   ```

2. **Heatmap Updates**: Correct (no double-counting)
   - Initial run: `{"other":1, "n8n":1, "docs":1}`
   - Re-run (same date): Counts unchanged ✅
   - Next day (2025-12-16): New entry added, previous unchanged ✅

3. **Obsidian API Connection**: Working
   - From host: ✅ `127.0.0.1:27124`
   - From n8n container: ✅ `host.docker.internal:27124`
   - Authentication: ✅ `MCP_OBSIDIAN_API_KEY` via Bearer token

4. **Cross-Platform Compatibility**: Verified
   - Mac (Docker Desktop): ✅ Tested
   - Linux: ✅ `extra_hosts` configuration added (requires Docker 20.10+)
   - Windows: ⚠️  Not tested (but `extra_hosts` is harmless)
...
**Verification**: Re-ran workflow 2x for 2025-12-15, counts stayed at 1 ✅
...
**Verification**: Script runs successfully from n8n container with exit code 0 ✅
...
**Verification**: Workflow execution history will show failures visibly ✅
...
**Application**:
- ❌ Bad: Script updates heatmap + n8n workflow also updates heatmap
- ✅ Good: Script is sole owner of `category_heatmap.json`
...
- Accumulation causes: running N times = N× the count (bug)
...
// ❌ Bad: Accumulation (not idempotent)
...
**Production Status**: ✅ Active (scheduled daily 00:30 JST)
