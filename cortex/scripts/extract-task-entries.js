  ...
  for (let i = 0; i < daysBack; i++) {
    ...
    try {
      ...
      if (brief.tasks && Array.isArray(brief.tasks)) {
        for (const task of brief.tasks) {
          ...
          entries.push({
            ...
            source: "brief",
            timestamp:
              brief.generated_at || brief.generated || date.toISOString(),
          });
        }
      }
    } catch (error) {
      ...
      if (error.code !== "ENOENT") {
        console.warn(
          `Warning: Could not process ${briefPath}: ${error.message}`,
        );
      }
    }
  }
...
async function enrichWithDigestContext(entries) {
  ...
  for (const [date, dateEntries] of Object.entries(entriesByDate)) {
    ...
    try {
      ...
      dateEntries.forEach((entry) => {
        entry.dayOfWeek = new Date(date).toLocaleDateString("en-US", {
          weekday: "long",
        });
