...
**Key Features**:
- 📌 Top 3 tasks for today (from `tomorrow.json`)
- ⏱ Time estimation with 20% buffer
- 🔄 Carryover tasks from yesterday
- 💭 Yesterday's reflection summary
...
```
☀️ *Morning Brief — 2025-12-03*

📌 *Today's Focus (Top 3)*
1. v1.2 Roadmap 確認 (1h)
2. Recipe 10 修正完了確認 ⚡ (1.5h)
3. Knowledge Graph タスク評価 🎯 (2h)

⏱ *Time Budget*: 4.5h + 1h buffer = 5.5h
...
|-----------|----------------|--------|
| `#urgent` / ⚡ | 1.5h | Time-critical, requires immediate focus |
| `#deepwork` / 🎯 | 2h | Deep concentration required |
...
```
Error: GET /vault/cortex/state/tomorrow.json → 404
```
**Solution**: 
...
**2. Empty task list**
```
Message: "（タスクなし - tomorrow.json を確認してください）"
```
**Solution**:
...
```
Error: POST to Slack webhook → 404/403
```
**Solution**:
...
curl -X POST $SLACK_DAILY_DIGEST_WEBHOOK \
  -H "Content-Type: application/json" \
  -d '{"text":"✅ Recipe 03 monitoring test"}'
```

### Weekly Review (5 minutes)

- Open n8n UI: http://localhost:5678
- Navigate to: Workflows → Recipe 03
...
- ✅ Time estimation with tag-based heuristics
- ✅ 20% buffer calculation
- ✅ Carryover tasks display
- ✅ Yesterday's reflection summary
- ✅ Slack DM notification
