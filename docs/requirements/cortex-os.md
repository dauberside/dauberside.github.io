...
- ✅ TODO auto-sync (Recipe 10)
- ✅ Daily digest generation (Recipe 14)
- ✅ Nightly wrap-up (Recipe 13)
...
**Acceptance Criteria**:
- Today's Focus → TODO.md sync (daily at 08:05 JST)
- Deduplication: identical tasks not duplicated
- Format consistency: `## Today — YYYY-MM-DD`
...
Recipe 13 (22:00) → tomorrow.json
  ↓
Human reviews & edits Today's Focus.md (manual)
  ↓
Recipe 10 (08:05) → TODO.md
  ↓
Human works on tasks (manual)
  ↓
Recipe 14 (00:00) → Daily Digest
  ↓
...
|-----------|--------|-------|--------|--------|
| Weekly | Recipe 11 | Daily Digests | Weekly Summary | ✅ Active |
...
### High Priority
- [ ] [Cortex] ⚡ Critical task  <!-- #urgent,#deepwork -->
- [ ] [n8n] 🚧 Blocked task  <!-- #blocked -->

### Regular Tasks
- [ ] [Docs] 🎯 Review task  <!-- #review -->
...
- Section header: `## Today` → `## Today — YYYY-MM-DD`
- Tags: inline → HTML comments
- Categories: optional → required prefix
...
**Input**:
- `Today's Focus.md` (Obsidian vault)
- `TODO.md` (existing)

**Process**:
1. GET `Today's Focus.md`
2. Extract uncompleted tasks (`- [ ]`)
3. Parse category, emoji, tags
4. GET `TODO.md`
5. Deduplicate (compare normalized text)
6. Build new section `## Today — YYYY-MM-DD`
7. Remove old `## Today —` section (regex)
...
2. Read TODO.md
3. Extract `## Today — YYYY-MM-DD` section
...
**Cost Estimation**:
- 146 files × 5 chunks/file = 730 chunks
- 730 chunks × 300 tokens/chunk = 219,000 tokens
...
**Breaking Changes**:
1. TODO format: `## Today` → `## Today — YYYY-MM-DD`
2. Tags: inline → HTML comments
...
**Required**:
- Recipe 02: KB rebuild → verify embeddings.json
- Recipe 10: TODO sync → verify format
- Recipe 14: Digest gen → verify output
