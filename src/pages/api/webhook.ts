...
async function tryCancelFromText(
  ...
  // 1) 直接ID/URLが文中にある場合はそれを優先
  {
    ...
    if (tokenLike) {
      try {
        ...
        if (groupId) {
          try { await pruneEventRefFromKV(groupId, tokenLike); } catch {}
        }
        await replyText(replyToken, `🗑 予定をキャンセルしました\nID: ${tokenLike}`);
      } catch (e: any) {
        await replyText(replyToken, `（取消失敗）${e?.message || '理由不明'}\nID: ${tokenLike}`);
      }
      return true;
    }
  }
  ...
  const placeWords = Array.from(new Set([...(dict.places || []), ...PLACE_WORDS]));
  const kws = Array.from(
    new Set([
      ...(extractHeuristicKeywordsJa(raw, placeWords) || []),
      guessSummaryJa(raw, undefined, extractLocationHeuristicJa(raw, placeWords), placeWords) || '',
      ...(dict.events || []),
    ].filter(Boolean))
  );
  ...
  const timeMax = fallbackRange.end || new Date(now.getTime() + 60 * 86400000).toISOString();
  ...
  const gMatched = (gcalEvents || []).filter((e) =>
    kws.length ? contains(`${e.summary || ''} ${e.location || ''} ${e.description || ''}`, kws) : true
  );

  // KVとGCalの候補を統合（ID重複排除）
  const byId = new Map<string, { id: string; summary?: string; start?: string }>();
  for (const r of kvMatched) if (r?.id) byId.set(String(r.id), { id: String(r.id), summary: r.summary, start: r.start });
  for (const e of gMatched) if (e?.id) byId.set(String(e.id), { id: String(e.id), summary: e.summary, start: e.start?.dateTime || e.start?.date });
  ...
  if (candidates.length > 1) {
    // 候補提示（テンプレ・ポストバック）
    const cols: CarouselColumn[] = candidates.slice(0, 10).map((c): CarouselColumn => {
      const line = `${(c.summary || '(無題)').slice(0, 30)}\n${formatJstShort(c.start || '')}`;
      return {
        text: truncateForButtons(line, 60),
        actions: [{ type: 'postback', label: 'これを取消', data: `action=cancel&id=${c.id}` }],
      };
    });
    const altList = candidates.slice(0, 10).map((c, i) => `${i + 1}) ${(c.summary || '(無題)').slice(0, 40)}`).join('\n');
    await replyTemplate(
      replyToken,
  ...
  try {
    ...
    if (groupId) {
      try { await pruneEventRefFromKV(groupId, target.id); } catch {}
    }
    await replyText(replyToken, `🗑 予定をキャンセルしました\n${target.summary || ''}\nID: ${target.id}`);
  } catch (e: any) {
    await replyText(replyToken, `（取消失敗）${e?.message || '理由不明'}\nID: ${target.id}`);
...
declare module '@/lib/interpret' {
  export function extractScheduleQuery(
    text: string
  ): Promise<{
    intent?: string;
...
let _extractScheduleReal: ((
  t: string
) => Promise<{ intent?: string; date_range?: { start?: string; end?: string }; keywords?: string[] }>) | null = null;
...
function coerceToJstWall(iso?: string): string {
  ...
  if (/T\d{2}:\d{2}/.test(s) && !/[+-]\d{2}:\d{2}$/.test(s)) return s + '+09:00';
  return s;
}
...
function formatJstShort(iso?: string): string {
  ...
  const jp = d.toLocaleString('ja-JP', { hour12: false, timeZone: 'Asia/Tokyo' }); // 例: 2025/8/21 12:34:56
...
// よく出る地名（ロケーション推定用）
const PLACE_WORDS = [
  '渋谷','新宿','池袋','恵比寿','表参道','原宿','銀座','丸の内','六本木','品川','東京','横浜','川崎',
  // プロジェクト固有の地名も随時追加
  'とをが'
];

// --- ユーザー辞書（KV: places/events） ---
async function loadUserDictKV(groupId?: string): Promise<{ places: string[]; events: string[] }> {
...
function extractLocationHeuristicJa(raw: string, placeWords: string[] = PLACE_WORDS): string | undefined {
  ...
  // 「にXにライブ/会議/…」
  const m = txt.match(/(?:に|へ)\s*([^ \u3000にへ@]+?)\s*に(?:ライブ|ﾗｲﾌﾞ|会議|ミーティング|mtg|打合せ|打ち合わせ|面談|飲み会|ランチ|食事|予定)/i);
  if (m) return m[1];
...
function seemsScheduleLike(raw: string) {
  return /(予定|スケジュール|会議|ミーティング|mtg|ランチ|食事|飲み|集合|空き|空いて|約束|打合せ|打ち合わせ|キャンセル|取り消|取消|中止|削除|消して|消す)/i.test(raw)
    || /(\d{1,2}[:：]\d{2})/.test(raw)
    || /(今日|明日|明後日|今週[日月火水木金土]|来週[日月火水木金土]|\d{1,2}[\/月]\d{1,2}日?)/.test(raw)
    || /(入れておいて|入れて|入れといて|入れとく|入れる|追加|登録して|予約して)/.test(raw);
}

// ざっくり日本語からキーワードを拾う（空白が無い文章向け）
function extractHeuristicKeywordsJa(raw: string, placeWords: string[] = PLACE_WORDS): string[] {
  const keys: string[] = [];
  const dict = [
    '食事','ランチ','ディナー','会食','飲み','飲み会','打合せ','打ち合わせ','会議','ミーティング','mtg',
    'ライブ','ﾗｲﾌﾞ',
    '花火大会','誕生日','旅行','帰省','発表','展示','撮影','映画','歓迎会','送別会',
    ...placeWords
  ];
...
function guessSummaryJa(raw: string, keywords: string[] = [], locationGuess?: string, placeWords: string[] = PLACE_WORDS): string | undefined {
  if (!raw) return;
  ...
  if (locationGuess) {
    try {
      t = t.replace(new RegExp(locationGuess.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g'), ' ');
    } catch {}
  }

  // 日付・時間系の除去
  t = t
    .replace(/今日|明日|明後日|今週[日月火水木金土]|来週[日月火水木金土]|今月|来月|再来月/gi, ' ')
  ...
  t = t
    .replace(/(の)?予定(を|は)?/g, ' ')
    .replace(/(入れておいて|入れて|入れといて|入れとく|追加|登録|作成|予約|とって|押さえ|セット|お願いします?|して|してね|しておいて|してくれ|頼む)/g, ' ');
  ...
  cand = cand.replace(/^(?:に|で|へ|と|を|は|が)\s*/,'').replace(/\s*(?:に|で|へ|と|を|は|が)$/,'');

  if (cand && !/^予[定約]?$/.test(cand) && cand.length >= 2 && !placeWords.includes(cand)) {
    return cand.slice(0, 80);
  }

  const primary = (keywords || []).find(k => !placeWords.includes(k) && !/(予定|ライブ|ﾗｲﾌﾞ)/i.test(k));
...
function parseAmPmJa(raw: string, baseHour?: number): number | undefined {
  const s = raw;
  if (/午後|PM/i.test(s)) {
    const h = baseHour ?? (s.match(/(\d{1,2})/) ? parseInt(RegExp.$1, 10) : undefined);
...
function extractDayRangeJa(raw: string, now = new Date()): { start?: string; end?: string } {
  ...
  const monthRange = (Y: number, M: number) => {
    ...
    return { start: isoDay(Y, M, 1, 0, 0, 0), end: isoDay(Y, M, last, 23, 59, 59) };
  };
  ...
  if (/再来月(?!.*\d{1,2}\s*日)/.test(text)) {
    let YY = y, MM = m + 2; while (MM > 12) { MM -= 12; YY += 1; }
    return monthRange(YY, MM);
  }
  ...
  if (/明日/.test(text)) {
    ...
    const de = isoDay(d.getFullYear(), d.getMonth() + 1, d.getDate(), 23, 59, 59);
    return { start: ds, end: de };
  }
  if (/明後日/.test(text)) {
...
async function handleScheduleIntent(
  ...
  if (!intent || intent === 'smalltalk') {
    ...
    if (/(キャンセル|取り消|取消|中止|削除|消して|消す|消去|破棄)/.test(text) && !/予定/.test(text)) intent = 'cancel_event';
    else if (/(変更|リスケ|ずらし|移動)/.test(text)) intent = 'reschedule_event';
    // 「予定の確認」「スケジュール確認」等を明示的に check_schedule にする
    else if (/((予定|スケジュール).{0,6}(確認|チェック)|(確認|チェック).{0,6}(予定|スケジュール))/i.test(text)) intent = 'check_schedule';
    else if (/(入れておいて|入れて|入れといて|入れとく|入れる|追加|登録|作成|予約|とって|押さえ|セット)/.test(text) || /(\d{1,2}[:：]\d{2})/.test(text)) intent = 'create_event';
    else if (/(ある|空き|空いて|予定|いつ|何時)/.test(text)) intent = 'check_schedule';
  }
  if (!startISO || !endISO) {
  ...
  if ((intent === 'check_schedule') && (!startISO || !endISO)) {
    ...
    if (!endISO)   endISO   = new Date(nowC.getTime() + 30 * 86400000).toISOString();
  }
  // 時間帯の推定（朝/昼/夜、または 19:00- など）
  const tw = extractTimeWindowJa(text);
  if (tw && startISO && endISO && (intent === 'create_event' || intent === 'reschedule_event' || intent === 'edit_event')) {
    const narrowed = applyTimeWindowToRange(startISO, endISO, tw);
    startISO = narrowed.start;
    endISO = narrowed.end;
  }
  if (!keywords.length) {
    try { keywords.push(...extractHeuristicKeywordsJa(text, placeWords)); } catch {}
  }
  ...
  if (locationGuess && !keywords.includes(locationGuess)) keywords.push(locationGuess);
  ...
  const matched = keywords.length
    ? events.filter(ev =>
        containsAny(`${ev.summary || ''} ${ev.location || ''} ${ev.description || ''}`, keywords)
      )
    : events;
  ...
  if (intent === 'create_event') {
    ...
    if (!startISO || !endISO) {
      await replyText(replyToken, '（登録不可）日時が曖昧です。具体的に「8/23 20:30-21:00 食事 @渋谷」のように送ってください。');
      return;
    }
    const summary = (
  ...
  if (intent === 'cancel_event') {
    ...
    if (matched.length > 1) {
      const altList = matched.slice(0, 10).map((ev, i) => `${i + 1}) ${(ev.summary || '(無題)').slice(0, 40)}`).join('\n');
      await replyTemplate(
        replyToken,
        {
          type: 'carousel',
          columns: matched.slice(0, 10).map((ev): CarouselColumn => {
            ...
            return {
              text: truncateForButtons(line, 60),
              actions: [{ type: 'postback', label: 'これを取消', data: `action=cancel&id=${ev.id}` }],
            };
          })
        },
        `（取消候補）\n${altList}`
    ...
    await deleteGoogleCalendarEvent({ calendarId, eventId: target.id! });
    if (groupOrRoomId) {
      try { await pruneEventRefFromKV(groupOrRoomId, target.id!); } catch {}
    }
    await replyText(replyToken, `🗑 予定をキャンセルしました\n${target.summary}`);
    return;
  }
  ...
  if (intent === 'reschedule_event' || intent === 'edit_event') {
    // 新しい時間が読めないとき
    if (!startISO || !endISO) {
      await replyText(replyToken, '（変更不可）新しい日時が曖昧です。「来週火曜 15:00-16:00に変更」のように送ってください。');
      return;
    }
    if (!matched.length) {
      await replyText(replyToken, '（変更）対象の予定が見つかりませんでした。タイトルや時間帯を含めてもう一度送ってください。');
      return;
    }
    if (matched.length > 1) {
      const tips = matched.slice(0, 5).map(ev => `• ${ev.id} ${ev.summary}`).join('\n');
      await replyText(replyToken, `（変更候補が複数）/cancelid <ID> で一度削除するか、タイトルをより具体的に指定してください：\n${tips}`);
      return;
    }
    const target = matched[0];
  ...
  await replyText(replyToken, '（メモ）予定の確認・登録・取消にしたい場合は、日時を含めて話しかけてください。');
}
...
function isPongTrigger(rawText: string, normalized: string) {
  ...
  if (['おけ', 'おｋ', 'ｏｋ', 'ok!', 'ｏｋ！', 'オーケー'].includes(normalized)) return true;
  if (/(status|ステータス|コード)\s*200/.test(rawText.toLowerCase())) return true;
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (process.env.SKIP_LINE_SIGNATURE === 'true') {
    console.warn('⚠ SKIP_LINE_SIGNATURE is true: skipping signature verification');
  } else {
    ...
    if (!ok) {
      console.error('✗ signature mismatch (verify failed). rawLength=', raw.length);
      res.status(401).end();
      return;
    }
  }
  ...
  for (const ev of body.events || []) {
    ...
    console.log('SourceID=', groupOrRoomId, 'isGroupLike=', isGroupLike, 'ALLOW=', allow);
    ...
    if (ev.type === 'postback' && ev.postback?.data && ev.replyToken) {
      ...
      if (action === 'cancel' && id) {
        try {
          ...
          await replyText(ev.replyToken, `🗑 予定をキャンセルしました\nID: ${id}`);
        } catch (e: any) {
          await replyText(ev.replyToken, `（取消失敗）${e?.message || '理由不明'}\nID: ${id}`);
        }
        continue;
      }
      ...
      if (action === 'create' || data.startsWith('CREATE_EVENT|')) {
        try {
          const rawPayload = action === 'create'
            ? (params.get('payload') || '')
            : data.slice('CREATE_EVENT|'.length);
          // Robust decode (handles single/double-encoded payloads)
          const once = (() => { try { return decodeURIComponent(rawPayload); } catch { return rawPayload; } })();
          const twice = (() => { try { return decodeURIComponent(once); } catch { return once; } })();

          let obj: any = null;
          try {
            obj = JSON.parse(twice || '{}');
          } catch {
            try { obj = JSON.parse(once || '{}'); } catch { obj = null; }
          }
          ...
          console.log('CREATE_EVENT postback payload:', {
            ...
            fields: obj ? { summary: obj.summary, start: obj.start, end: obj.end, location: obj.location } : null,
          });

          if (!obj || !obj.start || !obj.end) {
            await replyText(ev.replyToken, '（登録不可）payload の解析に失敗しました。もう一度送ってください。');
            continue;
          }
          ...
          console.log('CREATE_EVENT normalized (JST wall):', { summary, startNorm, endNorm, location, description });
          ...
          console.log('createGoogleCalendarEvent request:', JSON.stringify(reqObj));
          const created = await createGoogleCalendarEvent(reqObj);
          if (!created || !created.id) {
            console.error('createGoogleCalendarEvent returned empty result', created);
            await replyText(ev.replyToken, '（登録失敗）カレンダー作成に失敗しました（空の応答）。');
            continue;
          }
          console.log('createGoogleCalendarEvent success:', { id: created.id, summary: created.summary, htmlLink: created.htmlLink });
          ...
          if (groupOrRoomId) {
            try {
              ...
              console.log('KV saveEventRefKV success:', { groupOrRoomId, id: created.id });
            } catch (e) {
              console.error('KV saveEventRefKV error, trying direct LPUSH fallback:', e);
              try {
                const key = eventListKey(groupOrRoomId);
                await (kv as any).lpush(key, JSON.stringify({
                  id: String(created.id),
                  summary,
                  start: startNorm,
                  end: endNorm,
            ...
            try { await cacheEventsToKV(groupOrRoomId, [created]); } catch {}
          }

          const dispStart = formatJst(created.start?.dateTime || created.start?.date);
          ...
          await replyText(ev.replyToken, `（登録失敗）${e?.message || 'payload解析に失敗しました'}`);
        }
        continue;
      }
    }
    ...
    if (ev.type === 'message' && ev.message?.type === 'text' && ev.replyToken) {
      ...
      if (await tryCancelFromText(text, groupOrRoomId, calendarId, ev.replyToken)) {
        continue;
      }
      ...
      if (/^修正\s*:/i.test(text)) {
        ...
        try {
          ...
          if (!startISO || !endISO) {
            await replyText(ev.replyToken, '（登録不可）日時が不足しています。`修正:{ "start": "...", "end": "..." }` を含めてください。');
            return;
          }
          ...
          await replyText(ev.replyToken, '（登録不可）修正データをJSONとして解析できませんでした。例: 修正:{"summary":"打合せ","start":"2025-09-01T15:00:00+09:00","end":"2025-09-01T16:00:00+09:00"}');
        }
        continue;
      }
      ...
      // /log [n]
      {
        ...
        if (m) {
          ...
          if (!items.length) {
            await replyText(ev.replyToken, '（ログ）保存されたメッセージはまだありません。');
          } else {
            const lines = items
              .slice()
              .reverse()
              .map((mm) => {
                ...
                const t = dt.toLocaleString('ja-JP', { hour12: false, timeZone: 'Asia/Tokyo' });
                return `• ${t} ${mm.text}`;
              })
              .join('\n');
            await replyText(ev.replyToken, `🗒 直近${items.length}件\n${lines}`);
          }
          continue;
        }
      }
      ...
      // /find <kw>
      {
        ...
        if (m) {
          ...
          if (!q) {
            ...
            const hits = (await searchMessagesKV(groupOrRoomId, q, 500)) as any[];
            if (!hits.length) {
              await replyText(ev.replyToken, `（検索）"${q}" は見つかりませんでした。`);
            } else {
              const lines = hits
                .slice(0, 10)
                .reverse()
                ...
                .join('\n');
              await replyText(ev.replyToken, `🔎 検索 "${q}" 上位${Math.min(10, hits.length)}件\n${lines}`);
            }
          }
          continue;
        }
      ...
      if (/^\/kvevents$/i.test(text)) {
        ...
        if (!clean.length) {
          ...
          const afterClean = (after || []).filter((r: any) => r && r.id && r.summary);
          if (!afterClean.length) {
            await replyText(ev.replyToken, `（KV）このトークの登録済みイベント記録はありません。\nkey=${key}`);
          } else {
            const lines = afterClean
              .map((r: any, i: number) => {
                const when = r.start
                  ? new Date(r.start).toLocaleString('ja-JP', { hour12: false, timeZone: 'Asia/Tokyo' })
                  : '';
                return `${i + 1}) ${String(r.id).slice(0, 10)}… ${r.summary || ''}${when ? ` (${when})` : ''}`;
              })
              .join('\n');
            await replyText(ev.replyToken, `（KV）直近イベント記録 上位${afterClean.length}件\nkey=${key}\n（GCalから取り込み: ${imported}件）\n${lines}`);
          }
        } else {
          const lines = clean
            .map((r: any, i: number) => {
            ...
            .join('\n');
          await replyText(ev.replyToken, `（KV）直近イベント記録 上位${clean.length}件\nkey=${key}\n${lines}`);
        }
        continue;
      }
          ...
          if (!rawArr || !rawArr.length) {
            ...
            if (rawArr.length === 1 && /^\s*\[/.test(String(rawArr[0] || ''))) {
              try {
                ...
                bodyTxt = (Array.isArray(arr) ? arr : []).slice(0, 5).map((v: any, i: number) => {
                  try {
                ...
                bodyTxt = rawArr.map((s, i) => `${i + 1}) ${String(s).slice(0, 280)}`).join('\n');
              }
            } else {
              bodyTxt = rawArr.map((s, i) => `${i + 1}) ${String(s).slice(0, 280)}`).join('\n');
            }
      ...
      if (/^\/kv$/i.test(text)) {
        if (!(await kvAvailable())) {
          ...
          try {
            // @ts-ignore (@vercel/kv may return number)
            const len = await (kv as any).llen(chatKey(groupOrRoomId));
            const peekRawArr = await kv.lrange<string>(chatKey(groupOrRoomId), 0, 0);
            let peekShort = '(none)';
            ...
            await replyText(ev.replyToken, `KV: host=${host}\nkey=${chatKey(groupOrRoomId)}\nlen=${len}\npeek=${peekShort}`);
          } catch (e) {
      ...
      if (/^\/wipe$/i.test(text)) {
        if (!(await kvAvailable())) {
          await replyText(ev.replyToken, '（ログ削除）KVが未設定です。VercelのStorage→KVを有効にしてください。');
        } else {
          try {
            await kv.del(chatKey(groupOrRoomId));
            await replyText(ev.replyToken, '（ログ削除）このトークの保存メッセージを削除しました。');
          } catch (e) {
      ...
      if (/^\/wipeevents$/i.test(text)) {
        if (!(await kvAvailable())) {
          await replyText(ev.replyToken, '（イベント記録削除）KVが未設定です。');
        } else {
          try {
            const key = eventListKey(groupOrRoomId);
            await kv.del(key);
            await replyText(ev.replyToken, `（イベント記録削除）このトークのカレンダー記録を削除しました。\nkey=${key}`);
          } catch (e) {
            console.error('KV wipeevents error', e);
            await replyText(ev.replyToken, '（イベント記録削除）失敗しました。');
          }
        }
        continue;
      }
      ...
      // /dict commands: ユーザー辞書を管理
      {
        ...
        if (m) {
          ...
          if (!mm) {
            await replyText(ev.replyToken, '使い方: /dict add place 渋谷 ／ /dict del event 花火大会 ／ /dict list');
            continue;
          }
          ...
          try {
            ...
            await replyText(ev.replyToken, '（辞書操作失敗）権限/接続をご確認ください。');
          }
          continue;
        }
      }
      ...
      if (/^#cal\?/i.test(text.trim())) {
        ...
        if (!evParsed) {
          await replyText(ev.replyToken, '（デバッグ）予定を読み取れませんでした。例: #cal? 8/22 14:00-15:00 打合せ @渋谷');
        } else {
      ...
      // /cancel <id|last|タイトルの一部>
      {
        ...
        if (m) {
          ...
          if (!refs.length) {
            await replyText(ev.replyToken, '（取消）このトークで直近に登録した予定の記録が見つかりません。');
            continue;
          }

          let target: null | { id: string; summary?: string; start?: string; ts?: number } = null;
          ...
          if (/^last$/i.test(arg)) {
            ...
            if (!target) {
              ...
              target = refs.find((r: any) => normStr(String(r.summary || '')).includes(key)) || null;
            }
          }
          ...
          try {
            await deleteGoogleCalendarEvent({ calendarId: process.env.CALENDAR_ID || 'primary', eventId: target.id! });
            await replyText(ev.replyToken, `🗑 予定をキャンセルしました\n${target.summary || ''}\nID: ${target.id}`);
            if (target.id) {
      ...
      // /cancelid <eventId>
      {
        ...
        if (m) {
            ...
            if (!eventId) {
              await replyText(ev.replyToken, '（取消失敗）有効なイベントID/URLを読み取れませんでした。');
            } else {
              try {
                await deleteGoogleCalendarEvent({ calendarId: process.env.CALENDAR_ID || 'primary', eventId });
      ...
      if (/^(#cal|#calendar|#予定)/i.test(text.trim().toLowerCase())) {
        ...
        const evParsed = await extractEventFromText(original);
        if (!evParsed) {
          await replyText(ev.replyToken, '予定をうまく読み取れませんでした。例: #cal 8/20 15:00-16:00 面談 @渋谷');
        } else {
          const calendarId = process.env.CALENDAR_ID || 'primary';
          const created = await createGoogleCalendarEvent({
            calendarId,
      ...
      if (/^\/ai\b/i.test(text)) {
        ...
        try {
          ...
          const scheduleIntents = new Set(['check_schedule','create_event','cancel_event','reschedule_event','edit_event']);
          if (parsed?.intent && scheduleIntents.has(parsed.intent)) {
            await handleScheduleIntent(q, ev.replyToken, process.env.CALENDAR_ID || 'primary', groupOrRoomId);
            continue;
          }
        } catch {}
        const wantSummary = /要約|まとめ|サマリ|ダイジェスト|整理/i.test(q);
        const wantSchedule =
          /予定|スケジュール|会議|ミーティング|mtg|ランチ|食事|空き|空いて|いつ|何時/i.test(q);
        ...
        if (wantSchedule) {
          try {
            ...
            const kw = (
              ...
              kwMatch.find(
                (w) =>
                  !/^(いつ|なに|何時|予定|スケジュール|会議|ミーティング|mtg|ランチ|食事|空き|空いて|キャンセル|取り消|中止)$/i.test(w)
              ) ||
              ''
            )
              .toString()
            ...
            if (events && events.length) {
              const lines = events
                ...
                .map((e: any) => {
                  ...
                  const s = sRaw ? new Date(sRaw).toLocaleString('ja-JP', {
                    hour12: false,
                    timeZone: 'Asia/Tokyo',
                  }) : '(未定)';
        ...
        let finalPrompt = q;
        try {
          const recent = (await getRecentMessagesKV(groupOrRoomId, 40)) as any[];
          if (recent && recent.length) {
            const bullets = recent.slice().reverse().map((m) => `- ${m.text}`).join('\n');
            ...
            if (wantSummary) {
              const transcript = recent
                ...
                .map((m) => {
                  ...
                  const t = dt.toLocaleString('ja-JP', { hour12: false, timeZone: 'Asia/Tokyo' });
                  const txt = String(m.text || '').replace(/\s+/g, ' ').slice(0, 300);
                  return `- [${t}] ${txt}`;
                })
                .join('\n')
                .slice(0, 3500);
        ...
        try {
          if (typeof isCfAiConfigured === 'function' && !isCfAiConfigured()) {
            await replyText(ev.replyToken, '（AI未設定）Cloudflare AI の環境変数が未設定です。管理者は CLOUDFLARE_ACCOUNT_ID / CLOUDFLARE_API_TOKEN / CF_AI_MODEL を設定してください。');
            continue;
          }
        } catch {}
        let answer: string;
        try {
          ...
          await replyText(ev.replyToken, '（AI応答不可）Cloudflare AI への接続に失敗しました。環境変数の設定をご確認ください。');
          continue;
        }
      ...
      if (seemsScheduleLike(text)) {
        try {
          await handleScheduleIntent(text, ev.replyToken, process.env.CALENDAR_ID || 'primary', groupOrRoomId);
          continue;
        } catch (e) {
