...
function shouldProxyToKbApi() {
  return process.env.KB_API_URL?.trim() || process.env.KB_API_PROXY === "1";
}
...
export default async function handler(
  ...
  try {
    if (req.method === "GET") {
      ...
      if (!parsed.success) {
        done(400, { phase: "validate", path: "GET" });
      ...
      const hits = await searchKB(q, { topK });
        ...
        path: "GET",
        proxy: res.getHeader("X-KB-Proxy") || "local",
        topK,
      });
      return res.status(200).json({ hits });
    }
    if (req.method === "POST") {
      ...
      if (!parsed.success) {
        done(400, { phase: "validate", path: "POST" });
        return res.status(400).json({ error: parsed.error.flatten() });
      }
