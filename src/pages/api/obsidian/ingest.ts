...
import type { IngestResponse } from "@/types/obsidian";
import { isIngestRequest } from "@/types/obsidian";
...
export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse<IngestResponse | { error: string }>,
) {
  if (req.method !== "POST") {
  ...
  if (!isIngestRequest(payload)) {
    return res
      .status(400)
      .json({ error: "Invalid payload: expected { mode, notes[] }" });
  }
  ...
  try {
    ...
    return res
      .status(500)
      .json({ error: `diff pipeline failed: ${e?.message || e}` });
  }
  ...
  const response: IngestResponse & {
    plan?: { path: string; chunks: number }[];
    message?: string;
    toEmbed?: number;
    skipped?: number;
    ...
    plan: diffResult.plan as { path: string; chunks: number }[] | undefined,
