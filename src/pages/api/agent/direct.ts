...
import {
  previewFromBuffer,
  previewFromFilePath,
} from "@/lib/agent/direct/attachments";
import { kbSearch } from "@/lib/agent/direct/kb";
import { buildUserMessage, SYSTEM_JA } from "@/lib/agent/direct/prompt";
import type { TextPreview } from "@/lib/agent/direct/types";
...
async function appendChatLog(line: Record<string, unknown>) {
  try {
    ...
    const file = path.join(
      dir,
      `chat-${new Date().toISOString().slice(0, 10)}.ndjson`,
    );
    const sanitize = (v: unknown) =>
      typeof v === "string" ? v.replace(/[\r\n]+/g, " ").slice(0, 4000) : v;
    const payload = Object.fromEntries(
      Object.entries(line).map(([k, v]) => [k, sanitize(v)]),
    );
...
export default async function handler(
  ...
  try {
    ...
    if (isMultipart) {
      ...
      message = String(
        fields.message || fields.text || fields.input || "",
      ).trim();
      tool = String(fields.tool || "").trim();
      try {
        toolParams = fields.Tool_Parameters
          ? JSON.parse(String(fields.Tool_Parameters))
          : {};
      } catch {
        toolParams = {};
      }
      ...
      for (const list of fileLists) {
        ...
        for (const f of arr) {
          ...
          if (f?.filepath) {
            previews.push(
              await previewFromFilePath(name, mime, size, f.filepath),
            );
          } else {
            previews.push({
              key: name,
      ...
      toolParams =
        typeof body.Tool_Parameters === "object"
          ? body.Tool_Parameters || {}
          : (() => {
              try {
                return JSON.parse(body.Tool_Parameters || "{}");
              } catch {
                return {};
              }
            })();
    ...
    let kbBlock = "";
    const wantKb =
      String((req.query.use_kb as string) || "").trim() === "1" ||
      tool === "kb_search";
    if (wantKb) {
    ...
    return res
      .status(500)
      .json({ error: "direct_agent_error", message: e?.message || String(e) });
  }
}
