...
export default async function handler(
  ...
  if (!envTokenRaw || normalize(envTokenRaw) === "") {
    console.error(
      `[api/agent/run] rid=${rid} missing INTERNAL_API_TOKEN on server`,
    );
  ...
  if (typeof last === "number" && now - last < RATE_LIMIT_WINDOW_MS) {
    console.warn(
      `[api/agent/run] rid=${rid} rate_limited wait=${RATE_LIMIT_WINDOW_MS - (now - last)}ms`,
    );
  ...
  const input =
    typeof (req.body as any)?.input === "string"
  ...
  if (!process.env.OPENAI_API_KEY) {
    ...
    return res.status(500).json({
      error: "missing OPENAI_API_KEY (set in .env.local and restart server)",
    });
  }
