...
export default async function handler(
  ...
  const done = (status: number, meta?: Record<string, any>) => {
    ...
    try {
      ...
      const ms = hiRes
        ? Number((t1 as bigint) - (t0 as bigint)) / 1e6
        : (t1 as number) - (t0 as number);
      const info = {
        method: req.method,
        status,
        ms: Math.max(0, ms).toFixed(1),
        rid,
        ...(meta || {}),
      } as any;
  ...
  let kb_snippets:
    | Array<{ source: string; text: string; score?: number | null }>
    | undefined = undefined;
  ...
  if (isMultipart) {
    ...
    try {
      ...
      const textFieldRaw = pickField(
        fields.input_as_text ?? fields.message ?? fields.text ?? fields.input,
      );
      input_as_text = typeof textFieldRaw === "string" ? textFieldRaw : "";
      const kbField = Array.isArray(fields.kb_snippets)
        ? fields.kb_snippets[0]
        : (fields.kb_snippets as any);
      if (typeof kbField === "string") {
        try {
          kb_snippets = JSON.parse(kbField);
        } catch {}
      }

      // minimal validation using Zod by constructing object
      const preCheck = TextWorkflowInputSchema.safeParse({
        input_as_text,
        kb_snippets,
      });
      if (!preCheck.success) {
        ...
        return res
          .status(400)
          .json({ error: "invalid_input", details: preCheck.error.flatten() });
      }
      ...
      if (fileAny) {
        ...
        const attachNote =
          `添付ファイル: ${fname} (${mtype}, ${Math.round(sz / 1024)}KB)` +
          (preview
            ? `\n内容プレビュー:\n${preview}`
            : "\n(このファイルはプレビューされていません)");
        const extra = {
          source: `attachment:${fname}`,
          text: attachNote,
        } as const;
        kb_snippets = Array.isArray(kb_snippets)
          ? [...kb_snippets, extra]
          : [extra];
      }
    } catch (e: any) {
    ...
    try {
      ...
      const alias =
        body?.input_as_text ?? body?.message ?? body?.text ?? body?.input ?? "";
      ...
      const parse = TextWorkflowInputSchema.safeParse({
        input_as_text,
        kb_snippets,
      });
      if (!parse.success) {
  ...
  if (process.env.NODE_ENV !== "production" && !process.env.OPENAI_API_KEY) {
    try {
      ...
      tryExtract(envLocal);
      if (!process.env.OPENAI_API_KEY) tryExtract(envFile);
    } catch {}
  }
  ...
  if (mockEnabled) {
    const mock = {
      output_text: `mock:${input_as_text}`,
      actions: [
        {
          type: "open_url",
          label: "KB を検索",
          url: `/api/kb/search?q=${encodeURIComponent(input_as_text.slice(0, 64))}`,
        },
      ],
    } as const;
    const outParse = TextWorkflowOutputSchema.safeParse(mock);
    if (!outParse.success) {
      done(500, { path: "mock", reason: "invalid_output" });
  ...
  if (!process.env.OPENAI_API_KEY) {
    console.error(
      `[api/agent/workflow-proxy] rid=${rid} missing OPENAI_API_KEY`,
    );
    const mockAllowed =
    ...
    if (mockAllowed) {
      ...
      const outParse = TextWorkflowOutputSchema.safeParse(mock);
      if (!outParse.success) {
        done(500, { path: "mock-fallback", reason: "invalid_output" });
        return res.status(500).json({ error: "mock_output_invalid" });
      }
