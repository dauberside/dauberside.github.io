...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!envTokenRaw || normalize(envTokenRaw) === '') {
    console.error(`[api/agent/workflow] rid=${rid} missing INTERNAL_API_TOKEN on server`);
  ...
  if (reqToken !== envToken) {
    ...
    console.warn(`[api/agent/workflow] rid=${rid} unauthorized reason=${reason}`);
  ...
  if (typeof last === 'number' && now - last < RATE_LIMIT_WINDOW_MS) {
    console.warn(`[api/agent/workflow] rid=${rid} rate_limited wait=${RATE_LIMIT_WINDOW_MS - (now - last)}ms`);
  ...
  if (mockEnabled) {
    console.info(`[api/agent/workflow] rid=${rid} mock=1 inputLen=${input_as_text.length}`);
    ...
    if (!outParse.success) return res.status(500).json({ error: 'mock_output_invalid' });
  ...
  try {
    console.info(`[api/agent/workflow] rid=${rid} exec inputLen=${input_as_text.length}`);
