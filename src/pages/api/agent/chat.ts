...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'GET') {
    ...
    const prodUrl = process.env.N8N_AGENT_URL || 'http://localhost:5678/webhook/mcp-agent-chat';
    const testUrl = process.env.N8N_AGENT_TEST_URL || prodUrl.replace('/webhook/', '/webhook-test/');
    ...
    return res.status(200).json({
      ...
      expected: {
        ...
        body: { message: 'こんにちは', sessionId: '<optional>', user: '<optional>', meta: { any: 'optional' } },
      },
      upstream: {
        production: prodUrl,
        test: testUrl,
      },
      session: { cookie: existingSid || null, header: req.headers['x-session-id'] || null },
      notes: [
  ...
  try {
    ...
    const prodUrl = process.env.N8N_AGENT_URL || 'http://localhost:5678/webhook/mcp-agent-chat';
    const testUrl = process.env.N8N_AGENT_TEST_URL || prodUrl.replace('/webhook/', '/webhook-test/');
    const useTest = req.query?.test === '1' || req.headers['x-n8n-test'] === '1';
    ...
    if (isMultipart) {
      ...
      if (process.env.N8N_API_TOKEN) headers['X-N8N-API-KEY'] = process.env.N8N_API_TOKEN;
      ...
      const doPost = (target: string) => fetch(target, { method: 'POST', headers, body: bodyBuf });
      ...
      const isJson = contentType.includes('application/json') || rawBody.trim().startsWith('{') || rawBody.trim().startsWith('[');
      if (isJson && rawBody) {
        try {
      ...
      let message: unknown = bodyObj.message ?? bodyObj.text ?? bodyObj.input ?? bodyObj.prompt ?? rawBody;
      const bodySid: unknown = bodyObj.sessionId ?? bodyObj.sessionID ?? bodyObj.sid;
      ...
      const headers: Record<string, string> = { 'Content-Type': 'application/json', 'Accept': 'application/json', 'X-Session-Id': sessionId };
      if (process.env.N8N_API_TOKEN) headers['X-N8N-API-KEY'] = process.env.N8N_API_TOKEN;
      if (process.env.N8N_BASIC_AUTH_ACTIVE === '1') {
        const u = process.env.N8N_BASIC_AUTH_USER || '';
        const p = process.env.N8N_BASIC_AUTH_PASSWORD || '';
      ...
      const doPost = () => fetch(url, { method: 'POST', headers, body: JSON.stringify(payload) });
    ...
    if (upstream.status === 404) {
      json = {
        ...
        message: 'The requested n8n webhook is not registered (is the workflow Active?).',
        tried: url,
        tip: useTest
    ...
    return res.status(500).json({ error: 'agent_proxy_error', message: e?.message || String(e) });
  }
}
