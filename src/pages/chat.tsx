...
const Chat = ({ initialMessages, error: initialError }) => {
            ...
            try {
                await channel.subscribe((status) => {
                    if (status === 'SUBSCRIBED') {
                        ...
                        console.error('リアルタイムサブスクリプションに失敗しました:', status);
                        setError(`リアルタイム更新の設定に失敗しました: ${status}。ページをリロードしてください。`);
                    }
                });
            }
            catch (err) {
                console.error('サブスクリプションエラー:', err);
                setError(`リアルタイム更新の設定中にエラーが発生しました: ${err instanceof Error ? err.message : '不明なエラー'}`);
            }
        };
    ...
    const handleSendMessage = useCallback(async (newMessage) => {
        ...
        setIsLoading(true);
        try {
            const { data, error } = await sendMessage(newMessage, userId, username || '匿名', false);
            if (error) {
                throw error;
            }
            ...
            setError(`メッセージの送信に失敗しました: ${error instanceof Error ? error.message : '不明なエラー'}`);
        }
        finally {
            setIsLoading(false);
        }
    ...
    const handleDeleteMessage = useCallback(async (messageId) => {
        ...
        try {
            ...
            setError(`メッセージの削除に失敗しました: ${error instanceof Error ? error.message : '不明なエラー'}`);
        }
        finally {
            setIsLoading(false);
        }
    ...
    return (<div className="min-h-screen bg-[rgb(0,14,40)] text-white">
      <Header className="mx-auto"/>
      <main className="container mx-auto px-4">
        <div className="mx-auto">
          <ChatBox messages={messages} currentUserId={userId} onDeleteMessage={handleDeleteMessage}/>
          <ChatInput username={username} onUsernameChange={handleUsernameChange} onSendMessage={handleSendMessage} isLoading={isLoading}/>
        </div>
      </main>
...
export const getServerSideProps = async () => {
    try {
        ...
        return {
            props: {
                initialMessages: [],
                error: error instanceof Error ? error.message : 'メッセージの取得中に予期せぬエラーが発生しました'
            }
        };
    }
};
