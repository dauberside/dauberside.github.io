...
export default function AgentWorkflowPage() {
  ...
  const [useTestWebhook, setUseTestWebhook] = React.useState(
    process.env.NEXT_PUBLIC_N8N_CHAT_TEST === "1",
  );
        ...
        try {
          ...
          if (resp.ok) {
            ...
            if (Array.isArray(data?.hits)) {
              kbSnippets = data.hits.map((h: any) => ({
                source: h.source,
                text: h.text,
                score: h.score,
              }));
    ...
    <div className="min-h-screen font-sans">
      ...
      <main className="w-full max-w-[1140px] mx-auto px-[50px] py-6">
        ...
        <div className="mt-4 text-xs text-white/60">
          ・サーバブリッジ経由（/api/agent/chat → n8n Webhook）で実行します。
          <br />
          ・テスト時は上のトグルで ?test=1 を付け、キャンバスの Waiting 中の
          Test URL へ送ります。
          <br />
          ・本番では mock は無効。OPENAI_API_KEY 未設定時は 500 になります。
          <br />
          ・簡易レート制限（500ms/トークン）により連続実行で 429
          となる場合があります。
          <br />
          ・添付: 画像/音声/テキスト/PDF/JSON
          を1件まで添付可能（10MBまで）。テキスト系は本文プレビューを文脈に注入します。
        </div>
      </main>
      <Footer />
    </div>
