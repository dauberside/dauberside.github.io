...
export const HeaderNew: React.FC<{ className?: string }> = ({ className = "" }) => {
		...
		const handler = (e: KeyboardEvent) => {
			if (e.key === "Escape") {
				...
				setTimeout(() => {
					...
					if (active && !menuRef.current?.contains(active) && active !== btnRef.current) close();
				}, 0);
			}
		};
					...
					<div id="logo" className="flex items-center gap-4">
						<Link href="/" legacyBehavior>
							<a className="inline-flex items-center">
								<Image src="/images/geometric_pattern.svg" alt="Logo" width={100} height={50} className="w-[30px] h-auto" />
							</a>
						</Link>
					</div>
					<nav>
						<ul className="flex items-center gap-5 md:gap-8 lg:gap-8">
							<li className="relative">
								...
								{open && (
									...
									>
										<li role="none">
											<Link href="/" legacyBehavior>
												<a role="menuitem" className="block w-full px-4 py-2 text-sm text-black hover:bg-gray-100 focus:bg-gray-100 outline-none hover:outline-none focus:outline-none focus-visible:outline-none no-underline hover:no-underline" onClick={close}>
													Home
												</a>
											</Link>
										</li>
							...
							<li >
								<Link href="https://www.instagram.com/DauberSide/" legacyBehavior>
									<a aria-label="Instagram" className="inline-flex items-center text-lg md:text-xl no-underline hover:no-underline">
										<FontAwesomeIcon icon={faInstagram} />
									</a>
								</Link>
							</li>
							<li>
								<button onClick={handleContactFormOpen} aria-label="Open contact form" className="p-0 bg-transparent inline-flex items-center text-lg md:text-xl outline-none focus:outline-none no-underline hover:no-underline">
