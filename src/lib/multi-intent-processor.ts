...
export class MultiIntentProcessor {
     ...
     */
    async processMultiIntent(text: string, context: ConversationContext): Promise<MultiIntentResult> {
        ...
        try {
            ...
            if (intents.length === 1) {
                const entities = await enhancedNLPProcessor.extractEntities(text, context);
                return this.createSingleIntentResult(intents[0], entities, Date.now() - startTime);
            }

            // Process compound intents
            return await this.processCompoundIntents(intents, text, context, startTime);
     ...
     */
    async detectMultipleIntents(text: string, context: ConversationContext): Promise<Intent[]> {
        try {
            // Use AI to detect multiple intents
            const prompt = this.buildMultiIntentPrompt(text, context);
            const response = await callCfChat(prompt, this.getMultiIntentSystemPrompt());

            const parsed = this.parseMultiIntentResponse(response);

            // Convert to Intent objects
            const intents: Intent[] = parsed.intents?.map((intentData: any) => ({
    ...
    private async processCompoundIntents(
        ...
        const relationships = await this.analyzeIntentRelationships(intents, text, context);

        // Create execution steps
        const executionSteps = await this.createExecutionSteps(intents, relationships, text, context);
        ...
        return {
            ...
            warnings,
            suggestions,
            canExecuteImmediately: this.canExecuteImmediately(compoundIntent, warnings),
            requiresUserConfirmation: this.requiresConfirmation(compoundIntent, warnings)
        };
    }
    ...
    private async createExecutionSteps(
        ...
        for (const intent of intents) {
            const entities = await this.extractEntitiesForIntent(intent, text, context);
     ...
     */
    private buildMultiIntentPrompt(text: string, context: ConversationContext): string {
        ...
        prompt += "この入力から複数の意図を検出し、以下のJSON形式で返してください:\n";
        prompt += `{
            "intents": [
                {
                    "name": "intent_name",
...
JSONフォーマットで正確に返答してください。`;
     ...
     */
    private async fallbackIntentDetection(text: string, context: ConversationContext): Promise<Intent[]> {
        const intents: Intent[] = [];

        // Simple pattern-based detection
        const patterns = [
            { pattern: /(作成|追加|新しい|スケジュール)/, category: IntentCategory.SCHEDULE_CREATE },
            { pattern: /(編集|変更|修正|更新)/, category: IntentCategory.SCHEDULE_EDIT },
            { pattern: /(削除|取消|キャンセル)/, category: IntentCategory.SCHEDULE_DELETE },
            { pattern: /(確認|検索|見る|表示)/, category: IntentCategory.SCHEDULE_QUERY },
            { pattern: /(設定|環境設定|プリファレンス)/, category: IntentCategory.PREFERENCE_CHANGE },
            { pattern: /(ヘルプ|助けて|使い方)/, category: IntentCategory.HELP_REQUEST }
        ];
    ...
    private calculateCompoundConfidence(intents: Intent[], relationships: IntentRelationship[]): number {
        ...
        return Math.max(0, Math.min(1, avgConfidence + relationshipBoost - complexityPenalty));
    }

    private determineComplexity(intents: Intent[], relationships: IntentRelationship[]): IntentComplexity {
        if (intents.length === 1) return IntentComplexity.SIMPLE;
        if (intents.length === 2 && relationships.length <= 1) return IntentComplexity.COMPOUND;
        if (relationships.some(r => r.type === RelationshipType.CONDITIONAL)) return IntentComplexity.ADVANCED;
        return IntentComplexity.COMPLEX;
    }

    private areRelatedIntents(category1: IntentCategory, category2: IntentCategory): boolean {
        const relatedGroups = [
            [IntentCategory.SCHEDULE_CREATE, IntentCategory.SCHEDULE_EDIT, IntentCategory.SCHEDULE_DELETE],
            [IntentCategory.SCHEDULE_QUERY, IntentCategory.SCHEDULE_EDIT],
            [IntentCategory.PREFERENCE_CHANGE, IntentCategory.HELP_REQUEST]
        ];
        return relatedGroups.some(group => group.includes(category1) && group.includes(category2));
    }

    private async extractEntitiesForIntent(intent: Intent, text: string, context: ConversationContext): Promise<Entity[]> {
        const allEntities = await enhancedNLPProcessor.extractEntities(text, context);
        return this.filterEntitiesForIntent(allEntities, intent);
    }

    private filterEntitiesForIntent(entities: Entity[], intent: Intent): Entity[] {
        const relevantTypes: Record<IntentCategory, EntityType[]> = {
            [IntentCategory.SCHEDULE_CREATE]: [EntityType.DATE_TIME, EntityType.LOCATION, EntityType.TITLE, EntityType.EVENT_TYPE],
            [IntentCategory.SCHEDULE_EDIT]: [EntityType.DATE_TIME, EntityType.LOCATION, EntityType.TITLE],
            [IntentCategory.SCHEDULE_DELETE]: [EntityType.TITLE, EntityType.DATE_TIME],
            [IntentCategory.SCHEDULE_QUERY]: [EntityType.DATE_TIME, EntityType.LOCATION, EntityType.TITLE],
    ...
    private getPrerequisites(intent: Intent, relationships: IntentRelationship[]): string[] {
        return relationships
            .filter(r => r.targetIntent === intent.name && r.dependency === DependencyType.REQUIRED)
    ...
    private calculatePriority(intent: Intent, relationships: IntentRelationship[]): ExecutionPriority {
        if (intent.category === IntentCategory.SCHEDULE_DELETE) return ExecutionPriority.HIGH;
        if (intent.category === IntentCategory.SCHEDULE_CREATE) return ExecutionPriority.NORMAL;
        return ExecutionPriority.LOW;
    }

    private canParallelize(intent: Intent, relationships: IntentRelationship[]): boolean {
    ...
    private sortExecutionSteps(steps: ExecutionStep[], relationships: IntentRelationship[]): ExecutionStep[] {
        // Simple topological sort
        return steps.sort((a, b) => b.priority - a.priority);
    }

    private async analyzeWarnings(compoundIntent: CompoundIntent, context: ConversationContext): Promise<ProcessingWarning[]> {
    ...
    private generateSuggestions(compoundIntent: CompoundIntent, warnings: ProcessingWarning[]): ExecutionSuggestion[] {
        ...
        if (compoundIntent.executionOrder.length > 2) {
            suggestions.push({
                ...
                applicableIntents: compoundIntent.executionOrder.map(s => s.intent.name),
    ...
    private findConflictingIntents(compoundIntent: CompoundIntent): string[] {
        ...
        const intents = [compoundIntent.primaryIntent, ...compoundIntent.secondaryIntents];

        // Check for create/delete conflicts
        const hasCreate = intents.some(i => i.category === IntentCategory.SCHEDULE_CREATE);
        const hasDelete = intents.some(i => i.category === IntentCategory.SCHEDULE_DELETE);
    ...
    private canExecuteImmediately(compoundIntent: CompoundIntent, warnings: ProcessingWarning[]): boolean {
        return warnings.every(w => w.severity !== WarningSeverity.CRITICAL);
    }

    private requiresConfirmation(compoundIntent: CompoundIntent, warnings: ProcessingWarning[]): boolean {
        return warnings.length > 0 || compoundIntent.complexity === IntentComplexity.ADVANCED;
    }
}
...
export async function processMultiIntent(text: string, context: ConversationContext): Promise<MultiIntentResult> {
    return await multiIntentProcessor.processMultiIntent(text, context);
}

export async function detectMultipleIntents(text: string, context: ConversationContext): Promise<Intent[]> {
