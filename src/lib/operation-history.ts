...
export class OperationHistoryManager {
    ...
    async recordOperation(
        ...
        const operation: Operation = {
            ...
            context,
            undoOperation: this.generateUndoOperation(type, beforeState, afterState, context)
        };
        ...
        try {
            ...
            console.log(`Operation recorded: ${operationId} - ${type} for user ${userId}`);
            return operationId;
        } catch (error) {
    ...
    async getOperationHistory(
        ...
        try {
            ...
            await stashPostbackPayload(historyKey, historyData, this.HISTORY_TTL_DAYS * 24 * 60 * 60);
            ...
            console.error(`Failed to get operation history for user ${userId}:`, error);
            return [];
        }
    }
     ...
     */
    async undoOperation(userId: string, operationId: string): Promise<UndoResult> {
        try {
            const operation = await this.getOperation(operationId);
            ...
            if (error instanceof Error && 'type' in error) {
                return {
                    success: false,
                    message: (error as unknown as SystemError).userMessage || (error as Error).message,
                    restoredState: null,
     ...
     */
    async bulkUndo(userId: string, timeRange: TimeRange): Promise<BulkUndoResult> {
        try {
            ...
            for (const operation of targetOperations.reverse()) {
                try {
                    ...
                    console.error(`Failed to undo operation ${operation.id} in bulk undo:`, error);
                    failedOperations.push(operation);
                }
            }
    ...
    async getOperation(operationId: string): Promise<Operation | null> {
        try {
            ...
            await stashPostbackPayload(operationKey, operationData, this.HISTORY_TTL_DAYS * 24 * 60 * 60);
     ...
     */
    private async addToUserHistory(userId: string, operationId: string): Promise<void> {
        const historyKey = `operation_history_${userId}`;
        ...
        try {
            ...
            await stashPostbackPayload(historyKey, JSON.stringify(operationIds), this.HISTORY_TTL_DAYS * 24 * 60 * 60);
        } catch (error) {
        ...
        if (!operation.undoOperation) {
            return {
                ...
                error: createSystemError(
                    ...
                    { userId: operation.userId, operationType: 'undo', operationStep: 'validation' }
                )
            };
        }
        ...
        try {
            ...
            return {
                ...
                error: createSystemError(
                    ...
                    { userId: operation.userId, operationType: 'undo', operationStep: 'execution' },
                    error as Error
                )
            };
        }
     ...
     */
    private getUndoRiskLevel(type: OperationType): 'safe' | 'caution' | 'warning' {
        switch (type) {
            case OperationType.DELETE_EVENT:
                return 'warning';
            case OperationType.CREATE_EVENT:
...
export async function undoUserOperation(userId: string, operationId: string): Promise<UndoResult> {
...
export async function getUserOperationHistory(
    ...
    return await operationHistoryManager.getOperationHistory(userId, limit, offset);
}

export async function bulkUndoOperations(userId: string, timeRange: TimeRange): Promise<BulkUndoResult> {
