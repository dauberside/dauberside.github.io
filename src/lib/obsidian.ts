...
const KEY = (process.env.OBSIDIAN_API_KEY || (process.env as any).obsidian_api_key || '').toString().trim();
...
async function obFetch<T = unknown>(path: string, init: RequestInit = {} as RequestInit): Promise<T> {
  ...
  const resp = await fetch(url, { ...init, headers, // @ts-ignore
    agent: httpsAgent });
  if (!resp.ok) throw new Error(`Obsidian API ${resp.status}: ${await resp.text().catch(()=> '')}`);
...
export async function listRoot() { return obFetch('/vault/'); }
