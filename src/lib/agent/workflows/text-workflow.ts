...
export type WorkflowOutput = { output_text: string; actions?: Array<{ type: 'open_url'|'call_api'|'navigate'|'copy'; label: string; url?: string; method?: 'GET'|'POST'; body?: any; }>; };
  ...
  const result = await runner.run(agent, conversationHistory);
    ...
    try {
      if (/vercel|デプロイ|deploy/.test(text)) {
        actions?.push({ type: 'open_url', label: 'Vercel ダッシュボードを開く', url: 'https://vercel.com/dashboard' });
      }
      if (/ログ|log|エラー/.test(text)) {
        actions?.push({ type: 'open_url', label: 'プロダクション Smoke 結果 (GitHub Actions)', url: 'https://github.com/dauberside/dauberside.github.io/actions/workflows/prod-smoke.yml' });
      }
      if (/kb|ドキュメント|docs/.test(text)) {
        const q = encodeURIComponent(workflow.input_as_text.slice(0, 64));
        actions?.push({ type: 'open_url', label: 'KB を検索', url: `/api/kb/search?q=${q}` });
      }
      if (/エージェント|agent|実行|run/.test(text)) {
        actions?.push({ type: 'call_api', label: '内部エージェント実行 (/api/agent/run)', url: '/api/agent/run', method: 'POST', body: { input: workflow.input_as_text } });
      }
      if (/スケジュール|会議|calendar|予定/.test(text)) {
        actions?.push({ type: 'open_url', label: 'Google カレンダーを開く', url: 'https://calendar.google.com/' });
      }
    } catch {}
