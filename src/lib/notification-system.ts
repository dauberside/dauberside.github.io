...
import { EventContext, WeatherInfo, TrafficInfo } from "./smart-reminder-engine";
...
export class CustomizableNotificationSystem {
    ...
    private readonly DEFAULT_TEMPLATES: NotificationTemplate[] = [
        {
            ...
            template: '⏰ {{reminder.minutesBefore}}分後に予定です: {{event.summary}}\n開始: {{event.start | formatDate("MM/dd HH:mm")}}\n{{#if event.location}}場所: {{event.location}}{{/if}}',
            variables: [
                { name: 'event.summary', type: 'string', description: 'イベントタイトル', required: true },
                { name: 'event.start', type: 'date', description: '開始時刻', required: true },
                { name: 'event.location', type: 'string', description: '場所', required: false },
                { name: 'reminder.minutesBefore', type: 'number', description: 'リマインダー時間（分前）', required: true }
            ],
        ...
        },
        {
            ...
            template: '🌤️ {{reminder.minutesBefore}}分後に予定です: {{event.summary}}\n開始: {{event.start | formatDate("MM/dd HH:mm")}}\n天気: {{weather.condition}} {{weather.temperature}}°C\n{{#if weather.precipitation}}☔ 降水量: {{weather.precipitation}}mm{{/if}}\n{{#if weather.recommendation}}💡 {{weather.recommendation}}{{/if}}',
            variables: [
                { name: 'event.summary', type: 'string', description: 'イベントタイトル', required: true },
                { name: 'event.start', type: 'date', description: '開始時刻', required: true },
                { name: 'weather.condition', type: 'string', description: '天気状況', required: true },
                { name: 'weather.temperature', type: 'number', description: '気温', required: true },
                { name: 'weather.precipitation', type: 'number', description: '降水量', required: false },
                { name: 'weather.recommendation', type: 'string', description: '天気に関する推奨事項', required: false }
            ],
            priority: NotificationPriority.NORMAL,
            isDefault: true,
            isCustom: false,
        ...
        },
        {
            ...
            template: '🚗 出発時間です: {{event.summary}}\n開始: {{event.start | formatDate("MM/dd HH:mm")}}\n{{#if event.location}}目的地: {{event.location}}{{/if}}\n🛣️ 移動時間: {{traffic.durationInTraffic}}分\n{{#if traffic.recommendation}}{{traffic.recommendation}}{{/if}}',
            variables: [
                { name: 'event.summary', type: 'string', description: 'イベントタイトル', required: true },
                { name: 'event.start', type: 'date', description: '開始時刻', required: true },
                { name: 'event.location', type: 'string', description: '場所', required: false },
                { name: 'traffic.durationInTraffic', type: 'number', description: '交通渋滞を考慮した移動時間', required: true },
                { name: 'traffic.recommendation', type: 'string', description: '交通に関する推奨事項', required: false }
            ],
        ...
        },
        {
            ...
            template: '🔔 準備時間です: {{event.summary}}\n開始: {{event.start | formatDate("MM/dd HH:mm")}}\n準備時間: {{reminder.preparationMinutes}}分\n{{#if event.description}}詳細: {{event.description}}{{/if}}',
            variables: [
                { name: 'event.summary', type: 'string', description: 'イベントタイトル', required: true },
                { name: 'event.start', type: 'date', description: '開始時刻', required: true },
                { name: 'reminder.preparationMinutes', type: 'number', description: '準備時間（分）', required: true },
                { name: 'event.description', type: 'string', description: 'イベント詳細', required: false }
            ],
            priority: NotificationPriority.NORMAL,
            isDefault: true,
            isCustom: false,
    ...
    static getInstance(): CustomizableNotificationSystem {
        if (!CustomizableNotificationSystem.instance) {
            CustomizableNotificationSystem.instance = new CustomizableNotificationSystem();
        }
    ...
    async renderNotification(
        ...
        try {
            ...
            throw createSystemError(
                ...
                { userId, operationType: 'render_notification', operationStep: 'rendering', additionalData: { templateId } },
                error as Error
            );
        }
    }
    ...
    async sendNotification(
        ...
        try {
            // Render notification
            const message = await this.renderNotification(templateId, context, userId);

            // Get user preferences for delivery
            const preferences = await getUserPreferences(userId);
            const userDeliveryConfig = this.buildDeliveryConfig(preferences, deliveryConfig);

            // Check if notification should be batched
            if (await this.shouldBatchNotification(context, preferences)) {
                return await this.addToBatch(templateId, context, userId, userDeliveryConfig);
            }

            // Send immediately
            return await this.deliverNotification(message, context, userDeliveryConfig);
    ...
    async createCustomTemplate(
        template: Omit<NotificationTemplate, 'id' | 'isDefault' | 'isCustom' | 'createdAt' | 'updatedAt'>,
        userId: string
    ): Promise<string> {
        try {
    ...
    async updateCustomTemplate(
        ...
        try {
            ...
            throw createSystemError(
                ...
                { userId, operationType: 'update_template', operationStep: 'updating', additionalData: { templateId } },
                error as Error
            );
        }
    }
    ...
    async deleteCustomTemplate(templateId: string, userId: string): Promise<void> {
        try {
            ...
            throw createSystemError(
                ...
                { userId, operationType: 'delete_template', operationStep: 'deletion', additionalData: { templateId } },
                error as Error
            );
        }
    }
    ...
    async previewTemplate(
        ...
        try {
            ...
            throw createSystemError(
                ...
                { userId, operationType: 'preview_template', operationStep: 'previewing', additionalData: { templateId } },
                error as Error
            );
        }
    }
     ...
     */
    async configureQuietHours(quietHours: QuietHours, userId: string): Promise<void> {
        try {
            ...
            await stashPostbackPayload(key, JSON.stringify(quietHours), this.TEMPLATE_TTL);
            ...
            throw createSystemError(
                ...
                { userId, operationType: 'configure_quiet_hours', operationStep: 'configuration' },
                error as Error
            );
        }
    }
     ...
     */
    async configureDoNotDisturb(dndSettings: DoNotDisturbSettings, userId: string): Promise<void> {
        try {
            ...
            await stashPostbackPayload(key, JSON.stringify(dndSettings), this.TEMPLATE_TTL);
            ...
            throw createSystemError(
                ...
                { userId, operationType: 'configure_dnd', operationStep: 'configuration' },
                error as Error
            );
        }
    }
        ...
        try {
            ...
            if (dndSettings?.enabled) {
                ...
                if (dndSettings.endTime && Date.now() > dndSettings.endTime) {
                    ...
                    await this.configureDoNotDisturb({ ...dndSettings, enabled: false }, userId);
                } else {
                    // Check if notification is allowed during DND
                    const allowedDuringDND = this.isAllowedDuringDND(context, dndSettings);
                    if (!allowedDuringDND) {
            ...
            if (quietHours?.enabled) {
                ...
                if (inQuietHours) {
                    ...
                    const allowedDuringQuiet = this.isAllowedDuringQuietHours(context, quietHours);
     ...
     */
    private async renderTemplate(template: NotificationTemplate, context: NotificationContext): Promise<string> {
    ...
    private applyFormatter(value: any, formatter: string): string {
        if (formatter.startsWith('formatDate(')) {
            const format = formatter.match(/formatDate\("([^"]+)"\)/)?.[1] || 'MM/dd HH:mm';
            if (value) {
     ...
     * Get template by ID
     */
    private async getTemplate(templateId: string, userId: string): Promise<NotificationTemplate | null> {
     ...
     */
    private async storeTemplate(template: NotificationTemplate, userId: string): Promise<void> {
        const key = `notification_template_${userId}_${template.id}`;
        await stashPostbackPayload(key, JSON.stringify(template), this.TEMPLATE_TTL);
    }

    /**
     * Get user's custom templates
     */
    private async getUserCustomTemplates(userId: string): Promise<NotificationTemplate[]> {
     ...
     */
    private createSampleContext(partial: Partial<NotificationContext>): NotificationContext {
        const now = new Date();
     ...
     */
    private buildDeliveryConfig(preferences: any, override?: Partial<DeliveryConfig>): DeliveryConfig {
     ...
     * Check if notification should be batched
     */
    private async shouldBatchNotification(context: NotificationContext, preferences: any): Promise<boolean> {
     ...
     */
    private async getDoNotDisturbSettings(userId: string): Promise<DoNotDisturbSettings | null> {
        try {
            const key = `dnd_settings_${userId}`;
            const data = await popPostbackPayload(key);
     ...
     * Check if notification is allowed during quiet hours
     */
    private isAllowedDuringQuietHours(context: NotificationContext, quietHours: QuietHours): boolean {
        // Allow urgent notifications if configured
        if (quietHours.allowUrgent && context.reminder.priority === NotificationPriority.URGENT) {
            return true;
        }

        // Allow "critical" notifications if configured (mapped to highest available level)
        if (quietHours.allowCritical && context.reminder.priority === NotificationPriority.HIGH) {
            return true;
        }
     ...
     */
    private isAllowedDuringDND(context: NotificationContext, dndSettings: DoNotDisturbSettings): boolean {
     ...
     */
    private checkQuietHoursExceptions(context: NotificationContext, exceptions: QuietHoursException[]): boolean {
        for (const exception of exceptions) {
            switch (exception.type) {
                case 'eventType':
                    ...
                    break;
                case 'keyword':
                    if (context.event.summary.toLowerCase().includes(exception.value.toLowerCase())) {
                        return true;
                    }
                    break;
...
export const customizableNotificationSystem = CustomizableNotificationSystem.getInstance();
...
export async function renderNotification(
    ...
    return await customizableNotificationSystem.renderNotification(templateId, context, userId);
}
...
export async function sendNotification(
    ...
    return await customizableNotificationSystem.sendNotification(templateId, context, userId, deliveryConfig);
}
...
export async function createCustomTemplate(
    ...
    return await customizableNotificationSystem.createCustomTemplate(template, userId);
}

export async function configureQuietHours(quietHours: QuietHours, userId: string): Promise<void> {
    return await customizableNotificationSystem.configureQuietHours(quietHours, userId);
}

export async function configureDoNotDisturb(dndSettings: DoNotDisturbSettings, userId: string): Promise<void> {
    return await customizableNotificationSystem.configureDoNotDisturb(dndSettings, userId);
}