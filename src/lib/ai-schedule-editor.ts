...
import { updateGoogleCalendarEvent, listGoogleCalendarEvents } from "@/lib/gcal";
import { createEditSession, getEditSession, storeUserInput, getUserInput } from "@/lib/session-manager";
import type { GCalEvent, CreateEventInput } from "@/lib/types";
...
export async function processAITimeEdit(
    ...
    try {
        ...
        if (interpretation.start && interpretation.end) {
            ...
            const originalDuration = new Date(currentEnd).getTime() - new Date(currentStart).getTime();
            newStart = ensureJstIso(interpretation.start);
            newEnd = ensureJstIso(new Date(new Date(newStart).getTime() + originalDuration));
        } else {
            // 相対的な変更を検出
            ...
            if (relativeChange) {
                ...
                return {
                    success: false,
                    message: "❌ 時間変更の内容を特定できませんでした。より具体的に指定してください。"
                };
            }
        }
        ...
        if (new Date(newStart).getTime() >= new Date(newEnd).getTime()) {
            return {
                success: false,
                message: "❌ 開始時間が終了時間以降になっています。時間を見直してください。"
            };
        }
        ...
        if (hasConflict) {
            return {
                success: false,
                message: "❌ 指定された時間に他の予定があります。別の時間を指定してください。"
            };
        }
...
export async function processAILocationEdit(
    ...
    try {
        ...
        if (!newLocation) {
            return {
                success: false,
                message: "❌ 場所を特定できませんでした。より具体的に指定してください。"
            };
        }
...
export async function processAIEditWithSession(
    ...
    if (timeSession) {
        await storeUserInput(userId, "ai_edit_time", timeSession.eventId, userInput);
        return await processAITimeEdit(userId, timeSession.eventId, userInput);
    }

    if (locationSession) {
        await storeUserInput(userId, "ai_edit_location", locationSession.eventId, userInput);
        return await processAILocationEdit(userId, locationSession.eventId, userInput);
    }
