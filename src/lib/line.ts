...
function sanitizeTemplate(t: ButtonsTemplate | CarouselTemplate | ConfirmTemplate): ButtonsTemplate | CarouselTemplate | ConfirmTemplate {
  if (t.type === 'buttons') {
  ...
  // confirm
  return { ...t, text: clampText(t.text, LIMITS.confirm), actions: t.actions?.slice(0, 2) || [] };
}
...
export async function replyTemplate(
  ...
  const body = {
    replyToken,
    messages: [
      { type: 'template', altText: derivedAlt, template: safe },
    ],
  };
  await lineFetch(body);
}
...
export async function replyConfirm(
  ...
  const template: ConfirmTemplate = {
    ...
    actions: [
      ...
      cancel ? { type: 'message', label: cancel.label, text: cancel.text } : { type: 'message', label: 'キャンセル', text: 'キャンセル' },
    ],
  };
