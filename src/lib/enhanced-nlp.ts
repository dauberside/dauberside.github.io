...
import { AIScheduleDraft } from "./ai";
...
export class EnhancedNLPProcessor {
    ...
    async processInput(
        ...
        try {
            ...
            // Detect ambiguities
            const ambiguities = await this.detectAmbiguities(text, intent, entities, context);

            // Generate suggestions
            const suggestions = await this.generateSuggestions(text, intent, entities, ambiguities, context);

            // Calculate overall confidence
            const confidence = this.calculateOverallConfidence(intent, entities, ambiguities);
            ...
            return await this.fallbackProcessing(text, context, Date.now() - startTime);
        }
    }
     ...
     */
    async extractIntent(text: string, context: ConversationContext): Promise<Intent> {
        try {
            // Build context-aware prompt
            const contextPrompt = this.buildIntentPrompt(text, context);

            const response = await callCfChat(contextPrompt, this.getIntentSystemPrompt());
     ...
     */
    async extractEntities(text: string, context: ConversationContext): Promise<Entity[]> {
        try {
    ...
    async resolveAmbiguity(
        ...
        try {
            ...
            const preferenceResult = await this.disambiguateWithPreferences(text, context);
     ...
     */
    private buildIntentPrompt(text: string, context: ConversationContext): string {
        ...
        prompt += "上記のコンテキストを考慮して、ユーザーの意図を分析してください。";
...
コンテキストを活用して正確に分析してください。`;
    ...
    private getDefaultIntent(text: string): Intent {
        ...
        if (text.includes('予定') || text.includes('スケジュール')) {
            if (text.includes('作成') || text.includes('登録') || text.includes('追加')) {
                return {
            ...
            if (text.includes('変更') || text.includes('編集') || text.includes('修正')) {
                return {
                    name: 'schedule_edit',
                    confidence: 0.6,
            ...
            if (text.includes('確認') || text.includes('見る') || text.includes('チェック')) {
                return {
                    name: 'schedule_query',
                    confidence: 0.6,
    ...
    /**
        ...
        */
    private async extractDateTimeEntities(text: string, context: ConversationContext): Promise<Entity[]> {
        const entities: Entity[] = [];
        ...
        const patterns = [
            ...
            { regex: /(今週|来週|再来週)の?(月|火|水|木|金|土|日)曜?日?/g, type: EntityType.DATE_TIME },
     ...
     */
    private async extractLocationEntities(text: string, context: ConversationContext): Promise<Entity[]> {
        const entities: Entity[] = [];
        ...
        if (context.userPreferences?.defaults?.frequentLocations) {
            for (const location of context.userPreferences.defaults.frequentLocations) {
     ...
     */
    private async extractPersonEntities(text: string, context: ConversationContext): Promise<Entity[]> {
        const entities: Entity[] = [];

        // Person patterns (Japanese names and titles)
        const patterns = [
            { regex: /([田中|佐藤|鈴木|高橋|渡辺|伊藤|山本|中村|小林|加藤]さん)/g, confidence: 0.8 },
     ...
     */
    private async extractEventTypeEntities(text: string, context: ConversationContext): Promise<Entity[]> {
        const entities: Entity[] = [];

        // Event type patterns
        const eventTypes = [
            '会議', 'ミーティング', '打合せ', '面談', '研修', 'セミナー',
            '会食', '飲み会', '懇親会', 'プレゼン', '発表', '報告会'
        ];
    ...
    private async detectAmbiguities(
        ...
        const timeEntities = entities.filter(e => e.type === EntityType.DATE_TIME);
            ...
            for (const entity of timeEntities) {
                if (this.isTimeAmbiguous(entity.value)) {
                    ambiguities.push({
                        ...
                        possibleValues: await this.getTimeInterpretations(entity.value, context),
                        confidence: 0.8
                    });
                }
            }
     ...
     */
    private async normalizeDateTimeEntity(value: string, context: ConversationContext): Promise<any> {
     ...
     */
    private async getTimeInterpretations(timeValue: string, context: ConversationContext): Promise<string[]> {
     ...
     */
    private async getAlternativeIntents(text: string, context: ConversationContext): Promise<string[]> {
    ...
    private getMissingScheduleInfo(entities: Entity[]): string[] {
        ...
        if (!entityTypes.includes(EntityType.TITLE) && !entityTypes.includes(EntityType.EVENT_TYPE)) {
            missing.push('タイトル');
        }
...
export async function extractIntent(text: string, context: ConversationContext): Promise<Intent> {
    return await enhancedNLPProcessor.extractIntent(text, context);
}

export async function extractEntities(text: string, context: ConversationContext): Promise<Entity[]> {
