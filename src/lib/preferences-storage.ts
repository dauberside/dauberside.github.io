...
export class PreferencesStorageManager {
    ...
    async getUserPreferences(userId: string): Promise<UserPreferences> {
        try {
            ...
            await stashPostbackPayload(preferencesKey, preferencesData, this.STORAGE_TTL);

            // Merge with defaults to ensure all fields are present
            const mergedPreferences = PreferenceUtils.mergeWithDefaults(storedPreferences, userId);
    ...
    async resetPreferences(
        ...
        try {
            ...
            for (const field of preserveFields) {
                ...
                const value = field.split('.').reduce((cur: any, key: string) => cur?.[key], currentPreferences);
                if (value !== undefined) {
    ...
    async importPreferences(
        ...
        try {
            ...
            const validationErrors = PreferenceUtils.validatePreferences(importData.preferences);
            ...
            if (overwriteExisting) {
                ...
                targetPreferences = PreferenceUtils.mergeWithDefaults(importData.preferences, userId);
                importedFields.push(...Object.keys(importData.preferences));
            } else {
                // Merge with existing preferences
                const currentPreferences = await this.getUserPreferences(userId);
                targetPreferences = this.deepMerge(currentPreferences, importData.preferences);

                // Track what was imported vs skipped
                this.trackImportChanges(currentPreferences, importData.preferences, importedFields, skippedFields);
            }

            // Version compatibility check
            if (importData.version !== targetPreferences.version) {
                warnings.push(`Version mismatch: importing from ${importData.version} to ${targetPreferences.version}`);
            }

            // Save imported preferences
            const saveResult = await this.saveUserPreferences(userId, targetPreferences);

            return {
                success: saveResult.success,
                message: saveResult.success ? 'Preferences imported successfully' : 'Import failed',
                importedFields,
                skippedFields,
    ...
    async createPreferenceBackup(userId: string, reason: string): Promise<void> {
        try {
            ...
            // Store backup
            await stashPostbackPayload(backupKey, JSON.stringify(backup), this.BACKUP_TTL);
            ...
            console.error(`Failed to create preference backup for user ${userId}:`, error);
     ...
     */
    async restoreFromBackup(userId: string, backupId: string): Promise<PreferenceUpdateResult> {
        try {
            const backupKey = `preference_backup_${userId}_${backupId}`;
            const backupData = await popPostbackPayload(backupKey);
            ...
            console.error(`Failed to restore backup ${backupId} for user ${userId}:`, error);
    ...
    private sanitizePersonalData(preferences: UserPreferences): UserPreferences {
        ...
        sanitized.defaults.frequentLocations = sanitized.defaults.frequentLocations.map(loc => ({
            ...loc,
            name: 'Location',
            address: undefined,
     ...
     * Update backup index
     */
    private async updateBackupIndex(userId: string, backupId: string): Promise<void> {
        ...
        try {
            ...
            // Store updated index
            await stashPostbackPayload(indexKey, JSON.stringify(backupIds), this.BACKUP_TTL);
        } catch (error) {
...
export const preferencesStorageManager = PreferencesStorageManager.getInstance();
...
export async function saveUserPreferences(
    ...
    return await preferencesStorageManager.saveUserPreferences(userId, preferences);
}
