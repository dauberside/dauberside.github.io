...
const DEFAULT_RETRY_CONFIGS: Record<ErrorType, RetryConfig> = {
    ...
    [ErrorType.USER_INPUT_ERROR]: { maxAttempts: 0, baseDelay: 0, maxDelay: 0, backoffMultiplier: 1, jitter: false },
    [ErrorType.INVALID_DATE_TIME]: { maxAttempts: 0, baseDelay: 0, maxDelay: 0, backoffMultiplier: 1, jitter: false },
    [ErrorType.INVALID_DURATION]: { maxAttempts: 0, baseDelay: 0, maxDelay: 0, backoffMultiplier: 1, jitter: false },
    [ErrorType.MISSING_REQUIRED_FIELD]: { maxAttempts: 0, baseDelay: 0, maxDelay: 0, backoffMultiplier: 1, jitter: false },
    [ErrorType.SYSTEM_ERROR]: { maxAttempts: 1, baseDelay: 1000, maxDelay: 5000, backoffMultiplier: 2, jitter: true },
    [ErrorType.SESSION_ERROR]: { maxAttempts: 0, baseDelay: 0, maxDelay: 0, backoffMultiplier: 1, jitter: false },
    [ErrorType.DATA_CORRUPTION]: { maxAttempts: 0, baseDelay: 0, maxDelay: 0, backoffMultiplier: 1, jitter: false },
    [ErrorType.CONFIGURATION_ERROR]: { maxAttempts: 0, baseDelay: 0, maxDelay: 0, backoffMultiplier: 1, jitter: false },
    [ErrorType.EXTERNAL_API_ERROR]: { maxAttempts: 2, baseDelay: 1000, maxDelay: 8000, backoffMultiplier: 2, jitter: true },
    [ErrorType.CLOUDFLARE_AI_ERROR]: { maxAttempts: 2, baseDelay: 1000, maxDelay: 6000, backoffMultiplier: 2, jitter: true },
    [ErrorType.WEATHER_API_ERROR]: { maxAttempts: 1, baseDelay: 2000, maxDelay: 5000, backoffMultiplier: 2, jitter: true },
    [ErrorType.TRAFFIC_API_ERROR]: { maxAttempts: 1, baseDelay: 2000, maxDelay: 5000, backoffMultiplier: 2, jitter: true },
    [ErrorType.CONNECTION_ERROR]: { maxAttempts: 2, baseDelay: 1000, maxDelay: 8000, backoffMultiplier: 2, jitter: true },
    [ErrorType.AUTHENTICATION_ERROR]: { maxAttempts: 1, baseDelay: 1000, maxDelay: 3000, backoffMultiplier: 1.5, jitter: false },
    [ErrorType.AUTHORIZATION_ERROR]: { maxAttempts: 0, baseDelay: 0, maxDelay: 0, backoffMultiplier: 1, jitter: false },
    [ErrorType.TOKEN_EXPIRED]: { maxAttempts: 1, baseDelay: 500, maxDelay: 2000, backoffMultiplier: 2, jitter: false },
    [ErrorType.INVALID_CREDENTIALS]: { maxAttempts: 0, baseDelay: 0, maxDelay: 0, backoffMultiplier: 1, jitter: false },
    [ErrorType.QUOTA_EXCEEDED]: { maxAttempts: 0, baseDelay: 0, maxDelay: 0, backoffMultiplier: 1, jitter: false },
    [ErrorType.DATA_VALIDATION_ERROR]: { maxAttempts: 0, baseDelay: 0, maxDelay: 0, backoffMultiplier: 1, jitter: false },
    [ErrorType.SCHEMA_VALIDATION_ERROR]: { maxAttempts: 0, baseDelay: 0, maxDelay: 0, backoffMultiplier: 1, jitter: false },
    [ErrorType.BUSINESS_RULE_VIOLATION]: { maxAttempts: 0, baseDelay: 0, maxDelay: 0, backoffMultiplier: 1, jitter: false },
    [ErrorType.SCHEDULE_CONFLICT]: { maxAttempts: 0, baseDelay: 0, maxDelay: 0, backoffMultiplier: 1, jitter: false },
    [ErrorType.RESOURCE_CONFLICT]: { maxAttempts: 0, baseDelay: 0, maxDelay: 0, backoffMultiplier: 1, jitter: false },
    [ErrorType.CONSTRAINT_VIOLATION]: { maxAttempts: 0, baseDelay: 0, maxDelay: 0, backoffMultiplier: 1, jitter: false }
};
...
export class ErrorRecoveryManager {
    ...
    async handleError(
        ...
        for (const action of recoveryActions.filter(a => a.automated)) {
            try {
                ...
                console.error(`Automated recovery failed: ${action.type}`, recoveryError);
            }
        }
    ...
    async generateRecoveryActions(
        ...
        const rollbackPoints = this.rollbackPoints.get(context.sessionId || context.userId);
        ...
        return actions.sort((a, b) => b.estimatedSuccessRate - a.estimatedSuccessRate);
    }

    /**
     * Create a retry action with exponential backoff
     */
    private createRetryAction(error: SystemError, context: OperationContext): RecoveryAction {
     ...
     */
    private createRollbackAction(error: SystemError, context: OperationContext): RecoveryAction {
        return {
            ...
            execute: async (ctx: OperationContext) => {
                const rollbackResult = await this.rollbackToLastCheckpoint(ctx.sessionId || ctx.userId);
                return {
                    success: rollbackResult.success,
                    message: rollbackResult.message,
                    userMessage: rollbackResult.success ? '前の状態に戻しました' : 'ロールバックに失敗しました',
                    shouldRetry: false,
     ...
     */
    private createSessionRestartAction(error: SystemError, context: OperationContext): RecoveryAction {
        return {
            ...
            execute: async (ctx: OperationContext) => {
                ...
                return {
                    ...
                    userMessage: '新しいセッションを開始しました。操作を最初からやり直してください。',
                    shouldRetry: false,
                    newContext: {
                        ...ctx,
     ...
     */
    private createAlternativeTimeAction(error: SystemError, context: OperationContext): RecoveryAction {
        return {
     ...
     */
    private createCalendarReconnectAction(error: SystemError, context: OperationContext): RecoveryAction {
        return {
            type: RecoveryType.RETRY,
     ...
     */
    private createInputCorrectionAction(error: SystemError, context: OperationContext): RecoveryAction {
        return {
            ...
            execute: async (ctx: OperationContext) => {
                return {
                    ...
                    userMessage: '入力内容を確認して、正しい形式で再入力してください。例：「明日 15:00-16:00 会議」',
                    shouldRetry: false
                };
            }
        };
     ...
     */
    private createNetworkRetryAction(error: SystemError, context: OperationContext): RecoveryAction {
        return {
            type: RecoveryType.RETRY,
    ...
    async createRollbackPoint(
        ...
        await stashPostbackPayload(`rollback_${sessionId}`, JSON.stringify(points), 3600);
     ...
     */
    async rollbackToCheckpoint(sessionId: string, checkpointId: string): Promise<RollbackResult> {
        const points = this.rollbackPoints.get(sessionId) || [];
