...
export const PREFERENCE_VALIDATION_RULES: PreferenceValidationRule[] = [
  // UI preferences
  {
    field: "ui.preferredInteractionStyle",
    required: true,
    type: "string",
    allowedValues: ["quick_reply", "natural_language", "mixed"],
  },
  {
    field: "ui.dateFormat",
    required: true,
    type: "string",
    allowedValues: ["MM/DD", "DD/MM", "YYYY-MM-DD"],
  },
  {
    field: "ui.timeFormat",
    required: true,
  ...
  {
    field: "notifications.maxNotificationsPerHour",
    required: true,
    type: "number",
    min: 1,
    max: 60,
  ...
  // AI learning preferences
  {
    field: "aiLearning.dataRetentionDays",
    required: true,
    type: "number",
    min: 1,
    max: 365,
  ...
  {
    field: "aiLearning.maxLearningEntries",
    required: true,
    type: "number",
    min: 10,
    max: 10000,
  ...
  {
    field: "aiLearning.learningThreshold",
    required: true,
    type: "number",
    min: 1,
    max: 10,
  ...
  {
    field: "aiLearning.suggestionConfidenceThreshold",
    required: true,
    type: "number",
    min: 0.1,
    max: 1.0,
  ...
  // Default values
  {
    field: "defaults.defaultEventDuration",
    required: true,
    type: "number",
    min: 15,
    max: 480,
  ...
  {
    field: "defaults.defaultReminderTime",
    required: true,
    type: "number",
    min: 0,
    max: 1440,
  ...
  {
    field: "defaults.bufferTimeBetweenEvents",
    required: true,
    type: "number",
    min: 0,
    max: 60,
...
export class PreferenceUtils {
    ...
    return {
      ...
      notifications: {
        ...
        quietHours:
          ...
          DEFAULT_USER_PREFERENCES.notifications.quietHours,
        eventTypeSettings:
          userPreferences.notifications?.eventTypeSettings || [],
      ...
      defaults: {
        ...
        commonEventTypes:
          userPreferences.defaults?.commonEventTypes ||
          DEFAULT_USER_PREFERENCES.defaults.commonEventTypes,
        preferredMeetingTimes:
          userPreferences.defaults?.preferredMeetingTimes ||
          DEFAULT_USER_PREFERENCES.defaults.preferredMeetingTimes,
        workingHours: {
      ...
      enabledFeatures:
        userPreferences.enabledFeatures ||
        DEFAULT_USER_PREFERENCES.enabledFeatures,
  ...
  static hasFeatureEnabled(
    ...
    return (
      preferences.enabledFeatures.includes(feature) ||
  ...
  static getActiveQuietHours(
    ...
    for (const quietHour of preferences.notifications.quietHours) {
      ...
      if (quietHour.startTime > quietHour.endTime) {
        if (
          timeString >= quietHour.startTime ||
          timeString <= quietHour.endTime
        ) {
          return quietHour;
        }
      } else {
