...
export class ClarificationSystem {
    ...
    async generateClarificationRequest(
        ...
        try {
            ...
            const question = await this.generateClarificationQuestion(ambiguity, context);
            const options = await this.generateClarificationOptions(ambiguity, context);
            ...
            const request: ClarificationRequest = {
                ...
                context: {
                    ...
                    ambiguity,
                    relatedEntities,
                    conversationHistory: context.recentMessages.map(m => m.content).slice(-3)
                },
                priority,
            ...
            throw createSystemError(
                ...
                { userId: context.userId, operationType: 'clarification', operationStep: 'generation' },
                error as Error
            );
        }
    }
    ...
    async processClarificationResponse(
        ...
        try {
            ...
            const clarificationResponse: ClarificationResponse = {
                ...
                timestamp: Date.now(),
                responseTime: Date.now() - (request.timeout - (this.CLARIFICATION_TTL * 1000))
            };

            // Resolve ambiguity based on response
            const result = await this.resolveWithResponse(request, clarificationResponse);
    ...
    async collectUserFeedback(
        ...
        try {
            ...
            const feedback: UserFeedback = {
                ...
                feedbackType: feedbackData.feedbackType || FeedbackType.INTERPRETATION_CORRECT,
    ...
    async getLearningBasedSuggestions(
        ...
        try {
            ...
            return suggestions.sort((a, b) =>
                (b.confidence * Math.log(b.frequency + 1)) - (a.confidence * Math.log(a.frequency + 1))
            ).slice(0, 3);
        } catch (error) {
     ...
     */
    async generateDisambiguationUI(
    ...
    private async generateClarificationQuestion(
        ...
        const templates = questionTemplates[ambiguity.type] || ['詳細を教えてください'];
    ...
    private async resolveWithResponse(
        ...
        try {
            ...
            if (response.selectedOption && response.selectedOption !== 'other') {
                ...
                const option = request.options?.find(o => o.id === response.selectedOption);
                if (option) {
    ...
    /**
      * Store clarification request
      */
    private async storeClarificationRequest(request: ClarificationRequest): Promise<void> {
        const key = `clarification_${request.id}`;
        await stashPostbackPayload(key, JSON.stringify(request), this.CLARIFICATION_TTL);
    }

    /**
     * Get clarification request
     */
    private async getClarificationRequest(requestId: string): Promise<ClarificationRequest | null> {
        try {
    ...
    private async storeFeedback(feedback: UserFeedback): Promise<void> {
        ...
        await stashPostbackPayload(key, JSON.stringify(feedback), this.LEARNING_DATA_TTL);
    ...
    private async storeLearningData(
        ...
        try {
            ...
            if (existingData) {
                ...
                existingData.confidence = (existingData.confidence + result.confidence) / 2;

                const key = `learning_${existingData.userId}_${this.hashString(existingData.inputPattern)}`;
                await stashPostbackPayload(key, JSON.stringify(existingData), this.LEARNING_DATA_TTL);
            } else {
                // Store new learning data
                const key = `learning_${learningData.userId}_${this.hashString(learningData.inputPattern)}`;
                await stashPostbackPayload(key, JSON.stringify(learningData), this.LEARNING_DATA_TTL);
     ...
     */
    private async updateLearningFromFeedback(feedback: UserFeedback): Promise<void> {
        try {
            if (feedback.feedbackType === FeedbackType.INTERPRETATION_INCORRECT && feedback.userCorrection) {
    ...
    private async findSimilarLearningData(
        ...
        for (const data of learningData) {
            const similarity = this.calculateSimilarity(inputPattern, data.inputPattern);
     ...
     */
    private async addToFeedbackIndex(userId: string, feedbackId: string): Promise<void> {
        try {
            ...
            await stashPostbackPayload(indexKey, JSON.stringify(feedbackIds), this.LEARNING_DATA_TTL);
        } catch (error) {
     ...
     */
    private async addToLearningIndex(userId: string, learningKey: string): Promise<void> {
        try {
            const indexKey = `learning_index_${userId}`;
            const indexData = await popPostbackPayload(indexKey);
            ...
            if (!learningKeys.includes(learningKey)) {
                ...
                await stashPostbackPayload(indexKey, JSON.stringify(learningKeys), this.LEARNING_DATA_TTL);
            }
        } catch (error) {
    ...
    private formatOptionLabel(value: any, ambiguityType: AmbiguityType): string {
        switch (ambiguityType) {
            case AmbiguityType.TIME_AMBIGUOUS:
                return typeof value === 'string' ? value : `${value.hour}:${value.minute}`;
        ...
        switch (ambiguity.type) {
            case AmbiguityType.TIME_AMBIGUOUS:
                steps.push(
                    ...
                    { step: 2, question: '開始時刻を選択してください', type: 'time_picker' },
                    { step: 3, question: '終了時刻を選択してください', type: 'time_picker' }
                );
                break;

            case AmbiguityType.LOCATION_AMBIGUOUS:
                steps.push(
                    { step: 1, question: '場所のタイプを選択してください', type: 'category_select', options: ['オフィス', '会議室', 'オンライン', 'その他'] },
                    { step: 2, question: '具体的な場所を入力してください', type: 'text_input' }
                );
                break;

            default:
                steps.push(
                    { step: 1, question: '詳細を入力してください', type: 'text_input' }
                );
        }
    ...
    async getUserFeedbackStats(userId: string): Promise<any> {
        try {
            ...
            for (const feedbackId of feedbackIds.slice(-50)) { // Last 50 feedback items
                try {
                    const feedbackData = await popPostbackPayload(`feedback_${feedbackId}`);
                    if (feedbackData) {
                        ...
                        // Re-store feedback
                        await stashPostbackPayload(`feedback_${feedbackId}`, feedbackData, this.LEARNING_DATA_TTL);
                    }
                } catch (error) {
...
export async function processClarification(
    ...
    return await clarificationSystem.processClarificationResponse(requestId, response);
}
...
export async function getLearningBasedSuggestions(
    ...
    return await clarificationSystem.getLearningBasedSuggestions(userId, input, context);
}