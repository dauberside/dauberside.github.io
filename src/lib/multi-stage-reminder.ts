...
import { SmartReminder, EventContext, ReminderType, DeliveryStatus, ResponseType } from "./smart-reminder-engine";
...
export class MultiStageReminderManager {
        ...
        try {
            ...
            for (const stage of config.stages) {
                if (await this.shouldCreateStage(stage, eventContext, eventStart)) {
                    const stageReminderTime = eventStart - (stage.minutesBefore * 60 * 1000);

                    if (stageReminderTime > now) {
                        // Apply context-aware adjustments if needed
                        let adjustedTime = stageReminderTime;
                        if (stage.reminderType === ReminderType.DEPARTURE ||
                            ...
                            try {
                                ...
                                console.error('Context adjustment failed, using standard timing:', error);
                            }
                        }
            ...
            console.log(`Created ${reminderIds.length} multi-stage reminders for event ${eventId}`);
            return reminderIds;
            ...
            throw createSystemError(
                ...
                { userId, operationType: 'create_multi_stage', operationStep: 'creation', additionalData: { eventId } },
                error as Error
            );
        }
    }
    ...
    async snoozeStageReminder(
        ...
        userId: string
    ): Promise<{ success: boolean; nextReminderTime?: number; escalated?: boolean }> {
        try {
    ...
    async postponeEvent(
        ...
        try {
            ...
            const timeDifference = newEventTime - new Date(config.eventContext.location?.name || '').getTime();
    ...
    async getEscalationStatus(eventId: string): Promise<{
        ...
        try {
            ...
            const escalationStages = config.stages.filter(stage => stage.isEscalation);

            return {
                hasEscalation: config.escalationEnabled && escalationStages.length > 0,
                escalationStages,
                nextEscalation: escalationStages.length > 0
    ...
    private async buildMultiStageConfig(
        ...
        if (eventContext.eventType && preferences.notifications?.eventTypeSettings) {
            const eventTypeSettings = preferences.notifications.eventTypeSettings.find(
                (setting: EventTypeNotificationSettings) => setting.eventType === eventContext.eventType
            );

            if (eventTypeSettings && eventTypeSettings.reminderMinutes) {
                // Create custom stages based on event type settings
                const customStages: ReminderStage[] = eventTypeSettings.reminderMinutes.map((minutes: number, index: number) => ({
                    id: `custom_${eventContext.eventType}_${index}`,
                    name: `${minutes}分前 (${eventContext.eventType})`,
                    minutesBefore: minutes,
                    priority: eventTypeSettings.priority || NotificationPriority.NORMAL,
                    reminderType: ReminderType.STANDARD,
        ...
        stages = stages.filter(stage => this.evaluateStageConditions(stage, eventContext));
        ...
        return {
            ...
            eventContext,
            stages,
            escalationEnabled: eventContext.importance === 'critical' || eventContext.importance === 'high',
     ...
     * Evaluate stage conditions
     */
    private evaluateStageConditions(stage: ReminderStage, eventContext: EventContext): boolean {
        ...
        return stage.conditions.every(condition => {
            switch (condition.type) {
                case 'importance':
                    return this.evaluateCondition(eventContext.importance, condition.operator, condition.value);
                case 'eventType':
                    return this.evaluateCondition(eventContext.eventType, condition.operator, condition.value);
                default:
                    return true;
            }
        });
     ...
     */
    private mergeStages(defaultStages: ReminderStage[], customStages: ReminderStage[]): ReminderStage[] {
        const merged = [...defaultStages];

        for (const customStage of customStages) {
            const existingIndex = merged.findIndex(stage => stage.minutesBefore === customStage.minutesBefore);
            if (existingIndex >= 0) {
    ...
    private buildSnoozeSettings(preferences: any): SnoozeSettings {
        ...
        return {
            enabled: userSnoozeSettings?.enabled ?? this.DEFAULT_SNOOZE_SETTINGS.enabled,
            defaultMinutes: userSnoozeSettings?.defaultMinutes ?? this.DEFAULT_SNOOZE_SETTINGS.defaultMinutes,
            maxSnoozes: userSnoozeSettings?.maxSnoozes ?? this.DEFAULT_SNOOZE_SETTINGS.maxSnoozes,
            availableOptions: userSnoozeSettings?.availableOptions ?? this.DEFAULT_SNOOZE_SETTINGS.availableOptions,
            escalateAfterMaxSnoozes: userSnoozeSettings?.escalateAfterMaxSnoozes ?? this.DEFAULT_SNOOZE_SETTINGS.escalateAfterMaxSnoozes
        };
    }
     ...
     */
    private generateStageMessage(eventData: any, stage: ReminderStage, config: MultiStageConfig): string {
    ...
    private async createEscalationReminder(
        ...
        const escalationStage: ReminderStage = {
            ...
            reminderType: ReminderType.ESCALATION,
            isEscalation: true,
            customMessage: `🚨 重要: ${originalReminder.summary}\n\nスヌーズ回数が上限に達しました。\n開始時刻が近づいています。\n\n開始: ${new Date(originalReminder.eventStart).toLocaleString('ja-JP', {
                hour12: false,
                timeZone: 'Asia/Tokyo'
            })}`
        };
     ...
     */
    private calculateNextEscalationTime(escalationStages: ReminderStage[]): number {
     ...
     */
    private async getMultiStageConfig(eventId: string): Promise<MultiStageConfig | null> {
        try {
     ...
     * Store stage reminder
     */
    private async storeStageReminder(reminder: SmartReminder, stage: ReminderStage): Promise<void> {
     ...
     * Get stage reminder
     */
    private async getStageReminder(reminderId: string): Promise<SmartReminder | null> {
        try {
...
export const multiStageReminderManager = MultiStageReminderManager.getInstance();
...
export async function snoozeStageReminder(
    ...
    return await multiStageReminderManager.snoozeStageReminder(reminderId, snoozeMinutes, userId);
}
...
export async function postponeEvent(
    ...
    return await multiStageReminderManager.postponeEvent(eventId, newEventTime, userId);
}
