...
function removeFillerWords(s: string) {
  return s
    .replace(/[#／\/]cal\s*/i, '')
    .replace(/(?:予定登録|登録|予定を入れて|予定入れて|予定いれて|入れて|入れといて|お願い|お願いします|して|しておいて)\s*/g, '')
    .replace(/^\s*[、。・:：-]+\s*/, '')
    .trim();
}
function pickDateFromText(src: string, now = nowJst()) {
  ...
  if (mRel) {
    ...
    return { y: dt.getFullYear(), m: dt.getMonth() + 1, d: dt.getDate(), rest: text };
  }
...
function pickTimesFromText(src: string) {
  ...
  if (mRange) {
    ...
    return { startH: sH, startM: sM, endH: eH, endM: eM, rest: text, allday: false };
  }
  ...
  if (mStart) {
    ...
    return { startH: sH, startM: sM, endH: (sH + 1), endM: sM, rest: text, allday: false };
  }
...
export async function extractEventFromText(text: string): Promise<{
  ...
  {
    const m = cleaned.match(/(\d{1,2})[\/月](\d{1,2})日?\s+(\d{1,2})(?::|時)(\d{2}|半)\s*[〜~\-]\s*(\d{1,2})(?::|時)(\d{2}|半)\s+(.+?)(?:\s*@(.+))?$/);
    if (m) {
      ...
      const end = toJstIso(endDate.getFullYear(), endDate.getMonth() + 1, endDate.getDate(), endDate.getHours(), endDate.getMinutes(), 0);
  ...
  const start = toJstIso(y, mo, da, sFix.h, sFix.m, 0);
  const end = toJstIso(endDate.getFullYear(), endDate.getMonth() + 1, endDate.getDate(), endDate.getHours(), endDate.getMinutes(), times.allday ? 59 : 0);
