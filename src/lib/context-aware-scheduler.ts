...
import { EventContext, LocationInfo, WeatherInfo, TrafficInfo } from "./smart-reminder-engine";
...
export class ContextAwareScheduler {
    ...
    private weatherCache = new Map<string, { data: WeatherInfo; timestamp: number }>();
    private trafficCache = new Map<string, { data: TrafficInfo; timestamp: number }>();
    ...
    async calculateContextAwareReminder(
        ...
        try {
            ...
            if (eventContext.requiresTravel && eventContext.location) {
                ...
                const homeLocation: LocationInfo | undefined = (userPrefs.defaults?.homeLocation) ||
                    userPrefs.defaults?.frequentLocations?.find((loc: any) => loc.category === 'home');
     ...
     */
    async getWeatherInfo(location: LocationInfo, eventTime: Date): Promise<WeatherInfo | undefined> {
        try {
            ...
            if (data.forecast) {
                const dayForecast = data.forecast.forecastday.find(day => day.date === eventDate);
                ...
                if (dayForecast) {
                    ...
                    weatherInfo = {
                        condition: hourForecast?.condition.text || dayForecast.day.condition.text,
                        temperature: hourForecast?.temp_c || (dayForecast.day.maxtemp_c + dayForecast.day.mintemp_c) / 2,
                        precipitation: hourForecast?.precip_mm || dayForecast.day.totalprecip_mm,
                        ...
                        recommendation: this.generateWeatherRecommendation(
                            ...
                            hourForecast?.temp_c || (dayForecast.day.maxtemp_c + dayForecast.day.mintemp_c) / 2,
    ...
    private isOutdoorEvent(eventContext: EventContext): boolean {
        ...
        const outdoorKeywords = [
            '屋外', '公園', 'パーク', 'スポーツ', 'ゴルフ',
            'テニス', 'ランニング', 'ウォーキング', 'ハイキング',
            'バーベキュー', 'ピクニック', 'フェス', '祭り'
        ];

        const hasIndoorKeyword = indoorKeywords.some(keyword =>
            location.includes(keyword) || eventType.includes(keyword)
        );

        const hasOutdoorKeyword = outdoorKeywords.some(keyword =>
            location.includes(keyword) || eventType.includes(keyword)
        );

        return hasOutdoorKeyword || (!hasIndoorKeyword && eventContext.eventType !== 'meeting');
    }
    ...
    private calculateWeatherAdjustment(
        ...
        if (weather.temperature < 5) {
            ...
            recommendations.push('気温が低いため、防寒対策の時間を考慮して早めにリマインドします');
        ...
        if (weather.precipitation > 0.5) {
            ...
            recommendations.push('雨の予報があるため、雨具の準備と移動時間を考慮しました');

            if (weather.precipitation > 5) {
                adjustmentMinutes += 10;
                recommendations.push('強い雨の予報のため、さらに余裕を持った時間設定にしました');
            }
        }
        ...
        if (weather.alerts && weather.alerts.length > 0) {
            ...
            recommendations.push(`気象警報が発令されています: ${weather.alerts.join(', ')}`);
    ...
    private calculateTimeOfDayAdjustment(
        ...
        if ((hour >= 7 && hour <= 9) || (hour >= 17 && hour <= 19)) {
            if (eventContext.requiresTravel) {
                adjustmentMinutes += 15;
                recommendations.push('ラッシュアワーのため、移動時間に余裕を持たせました');
            }
        }
...
export async function getTrafficForEvent(
    ...
    return await contextAwareScheduler.getTrafficInfo(origin, destination, eventTime);
}