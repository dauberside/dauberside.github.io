...
get_feature_paths() {
    ...
    # Resolve template directory: prefer spec/templates, fallback to root templates
    if [ -d "$repo_root/spec/templates" ]; then
        TEMPLATE_DIR="$repo_root/spec/templates"
    else
    ...
    # Also expose TEMPLATE_DIR when using eval $(get_feature_paths)
